...
public class VerifiedMultiOutcomeDiscoveryDiagnosticsTests
{
    ...
    public void Publishes_specific_outside_reduced_universe_blocker_and_shadow_contract()
    {
        ...
        try
        {
            ...
            using var json=JsonDocument.Parse(File.ReadAllText(Path.Combine(root,"exports/latest/verified-multioutcome-discovery.json")));
            ...
            using var completion=JsonDocument.Parse(File.ReadAllText(Path.Combine(root,"exports/latest/verified-multioutcome-completion.json")));
