...
public class ExecutionSafetyTests
{
    ...
    {
        var o = MsOptions.Create(opt ?? new ExecutionOptions());
    ...
    [Fact] public void Risk_tracks_daily_notional(){ var o=MsOptions.Create(new ExecutionOptions()); var r=new RiskManager(o); var builder=new OrderPlanBuilder(o); var vres=new PreTradeValidationResult(true,PreTradeDecision.Approved,RejectionReason.None,[],0.01m,1,1,DateTime.UtcNow); var p=builder.Build(BaseOpp(),vres); r.RegisterPlannedExecution(p); Assert.True(r.GetRiskSnapshot().DailyNotional>0); }
    [Fact] public void Risk_blocks_exposure(){ var o=MsOptions.Create(new ExecutionOptions{MaxExposurePerMarket=1m}); var r=new RiskManager(o); Assert.False(r.CanOpenPosition(BaseOpp() with {Quantity=10m}, out _)); }
