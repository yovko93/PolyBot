...
import { API, HUB, getBotHealth, getBotStatus, getControls, getDiscoveryHealth, getDryRunOrderPlans, getEquity, getExecutionAudit, getMultiOutcomeDiagnostics, getOpportunities, getPositions, getRisk, getScannerStats, getTerminalLogs, getTradeLogs, getVerifiedBasketScreener, pauseScanner, resumeScanner, subscribeToBotEvents } from '../services/botApi';
...
export function useBotData() {
  ...
  const [discoveryHealth, setDiscoveryHealth] = useState<any>(null);
  ...
  return useMemo(() => ({ API, HUB, status, opps, positions, trades, risk, scanner, logs, equity, controls, multiOutcomeDiagnostics, verifiedBasketScreener, discoveryHealth, executionAudit, dryRunOrderPlans, connectionStatus, lastHeartbeat, lastUpdated, source, lastRestError, lastEvent, pauseScanner, resumeScanner }), [status, opps, positions, trades, risk, scanner, logs, equity, controls, multiOutcomeDiagnostics, verifiedBasketScreener, discoveryHealth, executionAudit, dryRunOrderPlans, connectionStatus, lastHeartbeat, lastUpdated, source, lastRestError, lastEvent]);
}
