...
export default function App() {
  ...
  const positiveCaptureRows = [
    ['Captures total', first(positiveCapture.capturesTotal, runtime(health, 'paperPhase1PositiveCapturesTotal'), 0)],
    ['Above min edge', first(positiveCapture.capturesAboveMinEdge, runtime(health, 'paperPhase1PositiveCapturesAboveMinEdge'), 0)],
    ['Paper eligible', first(positiveCapture.capturesPaperEligible, runtime(health, 'paperPhase1PositiveCapturesPaperEligible'), 0)],
    ['Opened', first(positiveCapture.capturesOpened, runtime(health, 'paperPhase1PositiveCapturesOpened'), 0)],
    ['Best captured edge', first(positiveCapture.bestCapturedAfterSafetyEdge, runtime(health, 'paperPhase1PositiveBestCapturedAfterSafetyEdge'), 'N/A')],
    ['Best blocking reason', first(positiveCapture.bestCapturedFirstBlockingReason, runtime(health, 'paperPhase1PositiveBestCapturedFirstBlockingReason'), 'None')],
    ['Export status', first(positiveCapture.exportWritten, runtime(health, 'paperPhase1PositiveCaptureExportWritten'), false) ? 'Written' : `Not written: ${first(positiveCapture.lastWriteError, runtime(health, 'paperPhase1PositiveCaptureLastWriteError'), 'None')}`],
    ...(positiveCaptures.length ? [] : [['Status', 'No positive real paper candidates captured.']])
  ];
