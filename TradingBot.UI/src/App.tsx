...
export default function App() {
  ...
  const release = dashboard?.paperPhase1ReleaseStatus ?? {};
  ...
  const releaseRows = [
    ...
    ['Waiting for edge / alert level', `${releaseState.alertName ?? 'WaitingForEdge'} / ${releaseState.alertLevel ?? 0}`],
    ['Best real-watch edge', releaseState.bestRealWatchAfterSafetyEdge ?? 'N/A'],
    ['Distance to min edge', releaseState.distanceToMinEdge ?? 'N/A'],
    ['Normal runtime positions', `${releasePositions.normalRuntimeOpened ?? 0} opened / ${releasePositions.normalRuntimeOpenPositions ?? 0} open`],
    ...
    ['Operator runbook available', String(release.operatorRunbookExportWritten ?? false)],
    ['Operator runbook last emitted', release.operatorRunbookLastEmittedUtc ?? '-'],
    ['Operator runbook export path', release.operatorRunbookExportPath ?? 'exports/paper-phase1-operator-runbook-latest.json'],
  ...
  const realMonitor = dashboard?.paperPhase1RealWatch ?? dashboard?.paperPhase1RealMonitor ?? {};
  const realLifecycle = dashboard?.paperPhase1RealLifecycle ?? {};
  const positiveReconciliation = dashboard?.paperPhase1PositiveReconciliation ?? {};
  const positiveCapture = dashboard?.paperPhase1PositiveCapture ?? {};
  const alertLabels = ['Waiting', 'Clean positive', 'Watch positive', 'Executable-like', 'Paper eligible', 'Paper opened'];
  const realAlertLevel = Number(runtime(health, 'paperPhase1RealAlertLevel') ?? 0);
  const realAlertRows = [
    ['Alert', `Level ${realAlertLevel} — ${alertLabels[realAlertLevel] ?? runtime(health, 'paperPhase1RealAlertName') ?? '-'}`],
    ['Reason', runtime(health, 'paperPhase1RealAlertReason') ?? '-'],
    ['Best real-watch edge', runtime(health, 'paperPhase1RealAlertAfterSafetyEdge') ?? 'N/A'],
    ['Distance to min edge', runtime(health, 'paperPhase1RealAlertDistanceToMinEdge') ?? 'N/A'],
    ['First blocking reason', runtime(health, 'paperPhase1RealAlertFirstBlockingReason') ?? 'None'],
    ['Stage counts', `invalid=${runtime(health, 'paperPhase1PositiveInvalidArtifactsTotal') ?? 0} clean=${runtime(health, 'paperPhase1PositiveCleanCapturesTotal') ?? 0} watch=${runtime(health, 'paperPhase1PositiveRealWatchTotal') ?? 0} executable=${runtime(health, 'paperPhase1PositiveExecutableLikeTotal') ?? 0} eligible=${runtime(health, 'paperPhase1PositivePaperEligibleTotal') ?? 0} opened=${runtime(health, 'paperPhase1PositiveOpenedTotal') ?? 0}`],
    ['Soak readiness / orderbook', `${runtime(health, 'paperPhase1RealSoakReadinessStable') ?? false} / ${runtime(health, 'paperPhase1RealSoakOrderbookStableMinutes') ?? 0} min`],
    ['Memory / log stable', `${runtime(health, 'paperPhase1RealSoakMemoryStable') ?? false} / ${runtime(health, 'paperPhase1RealSoakLogVolumeStable') ?? false}`],
    ['No signing / live attempts', `${runtime(health, 'paperPhase1RealSoakNoSigningAttempts') ?? false} / ${runtime(health, 'paperPhase1RealSoakNoLiveTradingBlocks') ?? false}`],
    ['Soak consistent', String(runtime(health, 'paperPhase1RealSoakConsistent') ?? false)]
  ];
  const contractFixtureEnabled = runtime(health, 'paperPhase1ContractFixtureEnabled') === true;
  const contractFixtureRows = [
    ['Fixture enabled', String(contractFixtureEnabled)],
    ['Dry replay only', String(runtime(health, 'paperPhase1ContractFixtureDryReplayOnly') ?? false)],
    ['Allow fixture paper open', String(runtime(health, 'paperPhase1ContractFixtureAllowPaperOpen') ?? false)],
    ['Stage reached', runtime(health, 'paperPhase1ContractFixtureStageReached') ?? 'None'],
    ['Alert level', runtime(health, 'paperPhase1ContractFixtureAlertLevel') ?? 0],
  ...
  const invalidArtifacts = Array.isArray(positiveCapture.topInvalidArtifacts) ? positiveCapture.topInvalidArtifacts : [];
  const validCaptureRows = [['Captures total', first(positiveCapture.validCapturesTotal, runtime(health, 'paperPhase1ValidPositiveCapturesTotal'), 0)], ['Above min edge', first(positiveCapture.validAboveMinEdge, runtime(health, 'paperPhase1ValidPositiveCapturesAboveMinEdge'), 0)], ['Clean positive but not watch-eligible', first(positiveCapture.cleanExcludedFromRealWatchTotal, runtime(health, 'paperPhase1CleanPositiveExcludedFromRealWatchTotal'), 0)], ['Watch-positive but not executable', Math.max(0, Number(positiveCapture.realWatchTotal ?? 0) - Number(positiveCapture.executableLikeTotal ?? 0))], ['Executable-like', first(positiveCapture.executableLikeTotal, runtime(health, 'paperPhase1PositiveExecutableLikeTotal'), 0)], ['Paper eligible', first(positiveCapture.paperEligibleTotal, runtime(health, 'paperPhase1PositivePaperEligibleTotal'), 0)], ['Opened', first(positiveCapture.openedTotal, runtime(health, 'paperPhase1PositiveOpenedTotal'), 0)], ['Best valid clean edge', first(positiveCapture.bestValidAfterSafetyEdge, runtime(health, 'bestValidCleanAfterSafetyEdge'), 'N/A')], ['Export status', first(positiveCapture.exportWritten, runtime(health, 'paperPhase1PositiveCaptureExportWritten'), false) ? 'Written' : 'Not written'], ...(positiveCaptures.length ? [] : [['Status', 'No valid positive real paper candidates captured.']])];
  const invalidArtifactRows = [['Artifacts total', first(positiveCapture.invalidArtifactsTotal, runtime(health, 'paperPhase1InvalidPositiveArtifactsTotal'), 0)], ['Above min edge', first(positiveCapture.invalidArtifactsAboveMinEdge, runtime(health, 'paperPhase1InvalidPositiveArtifactsAboveMinEdge'), 0)], ['Best artifact edge', first(positiveCapture.bestInvalidArtifactAfterSafetyEdge, runtime(health, 'bestInvalidArtifactAfterSafetyEdge'), 'N/A')], ['Best artifact reason', first(positiveCapture.bestInvalidArtifactFirstReason, runtime(health, 'paperPhase1InvalidPositiveArtifactBestFirstReason'), 'None')], ['Reason distribution', Object.entries(positiveCapture.invalidArtifactsByReason ?? {}).map(([k,v]) => `${k}=${v}`).join(' | ') || 'None'], ['Warning', 'High edge artifacts caused by missing ask / invalid snapshot are not openable.']];
  const positiveCaptureTable = useReactTable({ data: positiveCaptures, columns: [column.accessor('candidateId', { header: 'Candidate', cell: (i) => text(i.getValue()) }), column.accessor('question', { header: 'Market / question', cell: (i) => text(i.getValue()) }), column.accessor('afterSafetyEdge', { header: 'After-safety', cell: (i) => text(i.getValue()) }), column.accessor('firstBlockingReason', { header: 'First block', cell: (i) => text(i.getValue()) }), column.accessor('openedPositionId', { header: 'Paper position', cell: (i) => text(i.getValue() === 'None' ? '-' : i.getValue()) })], getCoreRowModel: getCoreRowModel() });
  const invalidArtifactTable = useReactTable({ data: invalidArtifacts, columns: [column.accessor('candidateId', { header: 'Candidate', cell: (i) => text(i.getValue()) }), column.accessor('question', { header: 'Market / question', cell: (i) => text(i.getValue()) }), column.accessor('afterSafetyEdge', { header: 'Artifact edge', cell: (i) => text(i.getValue()) }), column.accessor('firstBlockingReason', { header: 'Data-quality reason', cell: (i) => text(i.getValue()) })], getCoreRowModel: getCoreRowModel() });
  const formulaDiagnostics = dashboard?.formulaDiagnostics ?? {};
  const realMonitorRows = [
    ['Mode', realMonitor.canaryEnabled ? 'Canary profile (real scanner monitored separately)' : 'Real Monitor profile — no synthetic canary'],
    ['Status', realMonitor.armed && realMonitor.waitingForEdge ? 'Armed, waiting for real afterSafetyEdge >= 0.01' : (realMonitor.openPositions > 0 ? 'Real paper position open' : (realMonitor.armed ? 'Armed' : 'Not armed'))],
    ['Seen / valid priced', `${realMonitor.realCandidatesSeen ?? 0} / ${realMonitor.realValidPriced ?? 0}`],
    ['Near BE / positive', `${realMonitor.realNearBreakEven ?? 0} / ${realMonitor.realPositiveAfterSafety ?? 0}`],
    ['Eligible / attempts / opened', `${realMonitor.realPaperEligible ?? 0} / ${realMonitor.realOpenAttempts ?? 0} / ${realMonitor.realOpened ?? 0}`],
    ['Best after-safety', realMonitor.bestAfterSafetyEdge ?? 'N/A'],
    ['Distance to min edge', realMonitor.bestDistanceToMinEdge ?? 'N/A'],
    ['Top blocking reason', realMonitor.topBlockingReason ?? 'None'],
    ['Open attempts / succeeded / failed', `${realMonitor.openAttempts ?? 0} / ${realMonitor.openSucceeded ?? 0} / ${realMonitor.openFailed ?? 0}`],
    ['Opened position', realMonitor.openedPositionId ?? 'None'],
    ['Open / closed positions', `${realLifecycle.openPositionCount ?? 0} / ${realLifecycle.closedPositionCount ?? 0}`],
    ['Exposure / locked', `${money(Number(realLifecycle.exposure ?? 0))} / ${money(Number(realLifecycle.locked ?? 0))}`],
    ['Realized PnL', money(Number(realLifecycle.realizedPnl ?? 0))],
    ['Last settlement', `${realLifecycle.lastClosedPositionId ?? 'None'} — ${realLifecycle.lastSettlementReason ?? 'None'}`],
    ['Safety', `live disabled=${realMonitor.liveTradingDisabled ?? true}; signing disabled=${realMonitor.signingDisabled ?? true}`],
    ['Formula diagnostics', `blocking=${formulaDiagnostics.warningsBlocking ?? 0}; non-blocking=${formulaDiagnostics.warningsNonBlocking ?? 0}; suppressed=${formulaDiagnostics.warningsSuppressed ?? 0}`],
    ['Formula classification', formulaDiagnostics.lastWarningClassification ?? 'None'],
    ['Formula consistent', String(formulaDiagnostics.consistent ?? true)]
  ];
  const positiveReconciliationRows = [
    ['Status', positiveReconciliation.mismatchBlocking ? 'BLOCKED — unexplained positive candidate mismatch' : (positiveReconciliation.mismatch ? 'Explained candidate scope/gate mismatch' : 'Consistent')],
    ['Strategy positives / watch positives', `${positiveReconciliation.strategyPositiveAfterSafety ?? 0} / ${positiveReconciliation.realWatchPositiveAfterSafety ?? 0}`],
    ['Strategy best / watch best', `${positiveReconciliation.strategyBestAfterSafetyEdge ?? 'N/A'} / ${positiveReconciliation.realWatchBestAfterSafetyEdge ?? 'N/A'}`],
    ['Best candidate', positiveReconciliation.strategyBestCandidateId ?? 'None'],
    ['True blocking reason', positiveReconciliation.strategyBestCandidateReason ?? 'None'],
    ['Excluded total', positiveReconciliation.positiveExcludedTotal ?? 0],
    ['Excluded by reason', Object.entries(positiveReconciliation.positiveExcludedByReason ?? {}).map(([k,v]) => `${k}=${v}`).join(' | ') || 'None'],
    ['Mismatch reason', positiveReconciliation.mismatchReason ?? 'None'],
    ['Consistent', String(positiveReconciliation.consistent ?? true)]
  ];
  ...
  return <div className="terminal-root min-h-screen terminal-font">
    ...
    <main className="clean-dashboard">
      ...
      <Panel title="Console / Logs" active compact>
        <div className="diagnostics-grid">
          <MiniBlock title="Current logging" rows={[["Mode", runtime(health, 'consoleMode') ?? 'Summary5Min'], ["Summary interval", `${runtime(health, 'consoleSummaryIntervalSeconds') ?? 300}s`], ["Summaries written", runtime(health, 'consoleSummaryLogsWritten') ?? 0], ["Verbose suppressed / written", `${runtime(health, 'consoleVerboseEventsSuppressed') ?? 0} / ${runtime(health, 'consoleVerboseEventsWritten') ?? 0}`], ["Unexpected console events", runtime(health, 'consoleUnexpectedVerboseEventsPrinted') ?? 0], ["No-op alerts suppressed", runtime(health, 'consoleAlertNoopChangesSuppressed') ?? 0], ["Strict mode", String(runtime(health, 'consoleLoggingStrictModeOk') ?? true)], ["Verbose file", runtime(health, 'consoleVerboseLogPath') ?? 'exports/logs/verbose-events.jsonl'], ["Summary file", runtime(health, 'consoleSummaryLogPath') ?? 'exports/logs/summary.jsonl'], ["Last summary", time(runtime(health, 'consoleLastSummaryUtc'))]]} />
          <MiniBlock title="Copy commands" rows={[["Summary5Min", "dotnet run --project TradingBot -- --profile ReducedDiagnosticsPaperPhase1 --console-mode Summary5Min"], ["VerboseLegacy", "dotnet run --project TradingBot -- --profile ReducedDiagnosticsPaperPhase1 --console-mode VerboseLegacy"], ["Quiet", "dotnet run --project TradingBot -- --profile ReducedDiagnosticsPaperPhase1 --console-mode Quiet"]]} />
        </div>
      </Panel>
