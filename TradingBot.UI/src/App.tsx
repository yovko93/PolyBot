...
export default function App() {
  ...
  const validCaptureRows = [['Captures total', first(positiveCapture.validCapturesTotal, runtime(health, 'paperPhase1ValidPositiveCapturesTotal'), 0)], ['Above min edge', first(positiveCapture.validAboveMinEdge, runtime(health, 'paperPhase1ValidPositiveCapturesAboveMinEdge'), 0)], ['Paper eligible', first(positiveCapture.validPaperEligible, runtime(health, 'paperPhase1ValidPositiveCapturesPaperEligible'), 0)], ['Opened', first(positiveCapture.validOpened, runtime(health, 'paperPhase1ValidPositiveCapturesOpened'), 0)], ['Best valid clean edge', first(positiveCapture.bestValidAfterSafetyEdge, runtime(health, 'bestValidCleanAfterSafetyEdge'), 'N/A')], ['Export status', first(positiveCapture.exportWritten, runtime(health, 'paperPhase1PositiveCaptureExportWritten'), false) ? 'Written' : 'Not written'], ...(positiveCaptures.length ? [] : [['Status', 'No valid positive real paper candidates captured.']])];
