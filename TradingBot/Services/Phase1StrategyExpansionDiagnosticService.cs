...
public static class Phase1StrategyExpansionDiagnosticService
{
    ...
    private static string Diagnose(StrategyRow p,StrategyRow s,decimal min)=>(p.BestAfterSafetyEdge5m??decimal.MinValue)>=min?"PaperStrategyHasEdge":(s.BestAfterSafetyEdge5m??decimal.MinValue)>=min?"ShadowStrategyHasEdgeButNotPaperEligible":s.BlockedByPricing5m>0&&s.ValidPriced5m==0?"ShadowStrategyBlockedByPricing":s.BlockedByVerification5m>0&&s.ValidPriced5m==0?"ShadowStrategyBlockedByVerification":s.BlockedByDepth5m>0?"ShadowStrategyBlockedByDepth":s.BlockedByFill5m>0?"ShadowStrategyBlockedByFill":s.BlockedByRisk5m>0?"ShadowStrategyBlockedByRisk":"ShadowStrategiesAlsoBelowMinEdge";
