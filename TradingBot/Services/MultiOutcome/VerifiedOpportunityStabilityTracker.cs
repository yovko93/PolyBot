...
public enum VerifiedBasketState { NotExecutable, NearExecutable, EdgeExecutablePending, EdgeStable, ExecutionReadinessPending, ExecutionStable, ExperimentalProfilePending, ExperimentalProfileStable, DiagnosticsOnly, PaperOpened, SuppressedDuplicate }
...
public sealed class VerifiedOpportunityStabilityTracker
{
    ...
    public VerifiedBasketState TrackExperimental(string groupKey, decimal experimentalNetEdge, int requiredConsecutive, decimal minExperimentalNetEdge)
    {
        _consecutiveExperimental.TryGetValue(groupKey, out var c);
        if (experimentalNetEdge >= minExperimentalNetEdge) c++; else c = 0;
        _consecutiveExperimental[groupKey] = c;
        var state = c >= requiredConsecutive ? VerifiedBasketState.ExperimentalProfileStable : VerifiedBasketState.ExperimentalProfilePending;
        _state[groupKey] = state;
        return state;
    }

    public VerifiedBasketState TrackExperimental(string groupKey, VerifiedBasketScreener.ScreenResult row, int requiredConsecutive, decimal minExperimentalNetEdge)
        => TrackExperimental(groupKey, row.ExperimentalProfileNetEdge, requiredConsecutive, minExperimentalNetEdge);

    public int ConsecutiveExperimental(string groupKey) => _consecutiveExperimental.TryGetValue(groupKey, out var c) ? c : 0;

    public VerifiedBasketState MarkSuppressedDuplicate(string groupKey)
    {
        _state[groupKey] = VerifiedBasketState.SuppressedDuplicate;
        return VerifiedBasketState.SuppressedDuplicate;
    }

    public VerifiedBasketState MarkDiagnosticsOnly(string groupKey)
    {
        _state[groupKey] = VerifiedBasketState.DiagnosticsOnly;
        return VerifiedBasketState.DiagnosticsOnly;
    }
