...
public static class PaperPhase1EligibilityLadderExporter
{
    ...
    private static object ToNearEligible(SingleMarketOpportunityAuditDto x, int rank, decimal minEdge, long sourceCycle)
    {
        ...
        return new
        {
            ...
            edgeStable = !x.RejectedReason.Contains("Stable", StringComparison.OrdinalIgnoreCase),
            depthSufficient = x.DepthPassed,
            fillPassed = x.FillPassed,
            riskPassed = x.RiskPassed,
            paperEligible = x.AfterSafetyEdge >= minEdge && x.DepthPassed && x.FillPassed && x.RiskPassed && x.PaperDiagnosticsLimitedGatePassed,
