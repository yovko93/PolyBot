...
public sealed class PaperPhase1PositiveReconciliationService(TradingBotOptions options)
{
    ...
    private void Export(RuntimeHealthSnapshot h)
    {
        ...
        var payload = new { generatedAtUtc=DateTime.UtcNow, processRunId=ProcessRunContext.ProcessRunId,
            ...
            captureReconciliation=new { validPositiveCapturesTotal=PaperPhase1PositiveCaptureService.Current.ValidCapturesTotal,invalidPositiveArtifactsTotal=PaperPhase1PositiveCaptureService.Current.InvalidArtifactsTotal,bestValidPositiveAfterSafetyEdge=PaperPhase1PositiveCaptureService.Current.BestValidAfterSafetyEdge,bestInvalidArtifactAfterSafetyEdge=PaperPhase1PositiveCaptureService.Current.BestInvalidArtifactAfterSafetyEdge,bestInvalidArtifactReason=PaperPhase1PositiveCaptureService.Current.BestInvalidArtifactFirstReason },
    ...
    private void MaybeLog()
    {
        ...
        var captures=PaperPhase1PositiveCaptureService.Current;
        Console.WriteLine($"[PAPER_PHASE1_POSITIVE_RECONCILIATION] StrategyPositiveAfterSafety={Latest.StrategyPositiveAfterSafety} StrategyBestAfterSafetyEdge={Latest.StrategyBestAfterSafetyEdge:0.####} InvalidArtifactsTotal={captures.InvalidArtifactsTotal} CleanPositiveCapturesTotal={captures.ValidCapturesTotal} CleanPositiveAboveMinEdge={captures.ValidAboveMinEdge} CleanPositiveExcludedFromRealWatchTotal={captures.CleanExcludedFromRealWatchTotal} RealWatchPositiveTotal={captures.RealWatchTotal} ExecutableLikeTotal={captures.ExecutableLikeTotal} PaperEligibleTotal={captures.PaperEligibleTotal} BestCleanPositiveAfterSafetyEdge={captures.BestValidAfterSafetyEdge?.ToString("0.####")??"N/A"} BestCleanPositiveFirstBlockingReason={captures.BestValidFirstBlockingReason} BestRealWatchPositiveAfterSafetyEdge={captures.BestRealWatchAfterSafetyEdge?.ToString("0.####")??"N/A"} BestExecutableLikeAfterSafetyEdge={captures.BestExecutableLikeAfterSafetyEdge?.ToString("0.####")??"N/A"} MismatchBlocking={Latest.MismatchBlocking.ToString().ToLowerInvariant()} MismatchReason={Latest.MismatchReason} ProcessRunId={ProcessRunContext.ProcessRunId}");
