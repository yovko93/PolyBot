...
public sealed class PaperPhase1PositiveReconciliationService(TradingBotOptions options)
{
    ...
    private void MaybeLog()
    {
        ...
        Console.WriteLine($"[PAPER_PHASE1_POSITIVE_RECONCILIATION] StrategyPositiveAfterSafety={Latest.StrategyPositiveAfterSafety} StrategyBestAfterSafetyEdge={Latest.StrategyBestAfterSafetyEdge:0.####} RealWatchPositiveAfterSafety={Latest.RealWatchPositiveAfterSafety} ValidPositiveCapturesTotal={captures.ValidCapturesTotal} InvalidPositiveArtifactsTotal={captures.InvalidArtifactsTotal} BestValidPositiveAfterSafetyEdge={captures.BestValidAfterSafetyEdge?.ToString("0.####")??"N/A"} BestInvalidArtifactAfterSafetyEdge={captures.BestInvalidArtifactAfterSafetyEdge?.ToString("0.####")??"N/A"} BestInvalidArtifactReason={captures.BestInvalidArtifactFirstReason} MismatchBlocking={Latest.MismatchBlocking.ToString().ToLowerInvariant()} ProcessRunId={ProcessRunContext.ProcessRunId}");
