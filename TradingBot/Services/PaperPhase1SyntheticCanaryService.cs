...
public sealed class PaperPhase1SyntheticCanaryService(TradingBotOptions options, PaperTradingEngine paper, PaperPositionBook book, string contentRootPath)
{
    ...
    {
        if (_summaryLogged || !Current.Settled || state.PaperClosedPositions != 1 || state.PaperOpenPositions != 0 || !state.PaperCounterAuditConsistent) return;
        _summaryLogged = true;
        Console.WriteLine($"[PAPER_PHASE1_CANARY_FINAL_SUMMARY] Profile={options.RuntimeProfile} RunOnce={Current.RunOnce.ToString().ToLowerInvariant()} Attempted={Current.Attempted.ToString().ToLowerInvariant()} Opened={Current.Opened.ToString().ToLowerInvariant()} Settled={Current.Settled.ToString().ToLowerInvariant()} PositionId={Current.PositionId} CanaryOpenAttempts={Current.OpenAttempts} CanaryOpenSucceeded={Current.OpenSucceeded} CanaryPaperOpened={Current.PaperOpened} CanaryPaperClosed={Current.PaperClosed} GlobalOpenAttempts={state.PaperExecutionsCount} GlobalOpenSucceeded={state.PaperExecutionsCount} GlobalPaperOpened={state.PaperExecutionsCount} GlobalPaperClosed={state.PaperClosedPositions} GlobalOpenPositions={state.PaperOpenPositions} GlobalExposure={state.PaperTotalExposure:0.####} GlobalRealizedPnl={state.PaperRealizedPnl:0.####} CounterIncrementLogsWritten={state.PaperCounterIncrementLogsWritten} CounterDuplicateIncrementLogSuppressions={state.PaperCounterDuplicateIncrementLogSuppressions} RealOrderSent=false SigningAttempted=false Consistent={(Current.Consistent && state.PaperCounterAuditConsistent).ToString().ToLowerInvariant()} ProcessRunId={state.ProcessRunId}");
    }
    ...
    private void Export(BotRuntimeState state, PaperPosition? position, bool settled)
    {
        ...
        var written = SafeExportWriter.WriteJson(path, JsonSerializer.Serialize(payload, new JsonSerializerOptions { WriteIndented = true, PropertyNamingPolicy = JsonNamingPolicy.CamelCase }), "PaperPhase1SyntheticCanary", critical: false);
        var exportHealth = SafeExportWriter.Snapshot();
        Current = Current with { ExportWritten = written, LastWriteError = written ? "None" : $"{exportHealth.LastExceptionType}:{exportHealth.LastExceptionMessageShort}", ExportHealth = written ? "Ok" : exportHealth.Health };
    }
}

public sealed record PaperPhase1SyntheticCanaryState(bool Configured = false, bool Enabled = false, bool ProfileActive = false, string RequireProfile = "ReducedDiagnosticsPaperPhase1Canary", bool RunOnce = true, bool Attempted = false, bool Opened = false, bool Settled = false, bool Rejected = false, string RejectedReason = "Disabled", string PositionId = "None", decimal ExpectedProfit = 0m, decimal RealizedPnl = 0m, int DuplicateSuppressions = 0, string LastDuplicateReason = "None", int OpenAttempts = 0, int OpenSucceeded = 0, int PaperOpened = 0, int PaperClosed = 0, int OpenPositions = 0, decimal Exposure = 0m, DateTime? CanaryOpenAttemptUtc = null, DateTime? CanarySettledUtc = null, DateTime? OpenAttemptUtc = null, string ProcessRunId = "", bool ExportWritten = false, string LastWriteError = "None", string ExportHealth = "Ok")
{
