...
public static class VerifiedMultiOutcomeDiscoveryDiagnostics
{
    ...
    public static VerifiedDiscoverySnapshot Publish(string root, DateTime now)
    {
        lock (Sync)
        {
            ...
            var recommended=funnel.RawDiscovered5m>0&&funnel.EligibleForShadowEvaluation5m==0?"NoActiveOrderbookableVerifiedGroups":valid==0?"ExpandActiveVerifiedGroupDiscovery":positive==0?"ActiveGroupsBelowMinEdge":executable>0?"PromoteAfterFixtureTests":Recommend(top);
            var basePayload=new { TimeUtc=now,WindowStartUtc=_windowStart,WindowEndUtc=now,ReducedUniverseMarkets=_reducedUniverseMarkets,DiscoveryMode=_discoveryMode,MarketPoolCount=_marketPoolCount,CandidateGroupCount=discovered,VerifiedGroupCount=_groups.Count(g=>g.ValidationStatus=="VerifiedGroupResolved"),ActiveOrderbookableFunnel=funnel,EvaluatedGroupCount=evaluated,ValidPricedGroupCount=valid,PositiveAfterSafetyGroupCount=positive,ExecutableLikeGroupCount=executable,SkippedByReason=reasons,TopBlocker=top,RecommendedAction=recommended,TopSkippedOrBlockedGroups=samples };
            ...
            SafeExportWriter.AppendText(Path.Combine(root,"exports/phase1-shadow-orderbook-availability-history.jsonl"),JsonSerializer.Serialize(availabilityPayload)+Environment.NewLine);
