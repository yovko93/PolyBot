...
public static class Phase1ConsoleLogging
{
    ...
    private static long _invalidArtifactsWrittenBaseline,_invalidArtifactsDroppedBaseline;
        ...
        decimal? edge, bool paperEligible, decimal minEdge, string processRunId)
    {
        ...
        var stateChanged = previous.Level != next.Level
            ...
            || !string.Equals(previous.Reason?.Trim(), next.Reason?.Trim(), StringComparison.OrdinalIgnoreCase)
    ...
    public static void EmitSummary(TextWriter destination, RuntimeHealthSnapshot h, TradingBotOptions options)
    {
        ...
        var invalidArtifactsExport=RobustJsonlExportWriter.InvalidArtifactsSnapshot();
        ...
        var batchBookHealth=batchBooks.CircuitBreakerOpen?"CircuitOpen":batchBooks.LastErrorKind=="ProviderError"?"Unavailable":batchBooks.Errors>_batchBookBaseline.Errors?"Degraded":"Ok";
        ...
        var verifiedLiquidity = VerifiedMultiOutcomeLiquidityDiagnostics.Publish(_root,now);
        var shadowTokenAudit = ShadowTokenIdentityAudit.Publish(_root, now);
        var verifiedGroupFunnel = ShadowTokenIdentityAudit.GroupCurrent;
        ...
        var batchProviderStatus=shadowOrderbooks.Requests5m>0&&shadowOrderbooks.Missing5m==0&&shadowOrderbooks.LoadFailed5m==0
            ? "Ok" : batchBookHealth;
