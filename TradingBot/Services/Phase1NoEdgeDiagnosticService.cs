...
public static class Phase1NoEdgeDiagnosticService
{
    ...
    public static void Update(OpportunityFamilyRankingSnapshot ranking,FocusUniverseSnapshot focus,RuntimeHealthSnapshot h,TradingBotOptions options,string root)
    {
        ...
        CleanPositiveToGateReconciliationService.Update(rows,min,root);Phase1StrategyExpansionDiagnosticService.Observe(ranking,focus);}}
    ...
    private static void Write(string p,object v)=>SafeExportWriter.WriteJson(p,JsonSerializer.Serialize(v,new JsonSerializerOptions{WriteIndented=true,PropertyNamingPolicy=JsonNamingPolicy.CamelCase}));private static void Append(string p,object v){Directory.CreateDirectory(Path.GetDirectoryName(p)!);SafeExportWriter.AppendText(p,JsonSerializer.Serialize(v)+Environment.NewLine);}
}
