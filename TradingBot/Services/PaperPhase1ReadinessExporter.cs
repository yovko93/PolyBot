...
public static class PaperPhase1ReadinessExporter
{
    ...
    public static void ExportLatest(RuntimeHealthSnapshot h, string root)
    {
        try
        {
            ...
            SafeExportWriter.WriteText(path, json);
        }
    ...
    public static void MaybeLog(RuntimeHealthSnapshot h, TradingBotOptions options)
    {
        ...
        {
            _lastGateLogUtc = now;
            Console.WriteLine($"[PAPER_PHASE1_GATE_SUMMARY] CandidatesSeen={h.PaperPhase1CandidatesSeen} Rejected={h.PaperPhase1CandidatesRejected} Eligible={h.PaperPhase1CandidatesEligible} OpenAttempts={h.PaperPhase1OpenAttempts} OpenSucceeded={h.PaperPhase1OpenSucceeded} PaperOpened={h.PaperPhase1PaperOpened} LastRejectReason={h.PaperPhase1LastRejectReason} LastOpenBlockedReason={h.PaperPhase1LastOpenBlockedReason} ConsistencyOk={h.PaperPhase1ConsistencyOk.ToString().ToLowerInvariant()} FallbackAccepted={h.PaperPhase1DiscoveryFallbackAccepted.ToString().ToLowerInvariant()} UsingPersistedSnapshot={h.PaperPhase1UsingPersistedSnapshot.ToString().ToLowerInvariant()} ProcessRunId={h.ProcessRunId}");
        }
    }
}
