    ...
    IReadOnlyDictionary<string, TradingBot.Services.StrategyRuntimeCounterSnapshot> StrategyCounters)
{
    public string ToLogLine()
    {
        ...
        var strategies = string.Join(",", StrategyCounters.OrderBy(x => x.Key, StringComparer.OrdinalIgnoreCase).Select(x => $"{x.Key}:{x.Value.Mode}:scan={x.Value.Scanned}:books={x.Value.Books}:cand={x.Value.Candidates}:positive={x.Value.PositiveEdges}:ready={x.Value.ExecutionReady}:execCand={x.Value.ExecutionCandidates}:paper={x.Value.PaperOpened}:diagBlocked={x.Value.DiagnosticsOnlyBlocked}:faults={x.Value.Faults}"));
...
public static class RuntimeHealthTrendTracker
{
    ...
    public static string ToSoakStatusLogLine(RuntimeHealthSnapshot health, RuntimeHealthTrend trend, TradingBot.Options.TradingBotOptions? options = null, BotRuntimeState? state = null)
    {
        ...
        var strategyStatus = string.Join(",", health.StrategyCounters.OrderBy(x => x.Key, StringComparer.OrdinalIgnoreCase).Select(x => $"{x.Key}:{x.Value.Mode}:scan={x.Value.Scanned}:books={x.Value.Books}:cand={x.Value.Candidates}:positive={x.Value.PositiveEdges}:paper={x.Value.PaperOpened}:blocked={x.Value.DiagnosticsOnlyBlocked}:faults={x.Value.Faults}"));
