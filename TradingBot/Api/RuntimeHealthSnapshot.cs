    ...
    IReadOnlyDictionary<string, TradingBot.Services.StrategyRuntimeCounterSnapshot> StrategyCounters)
{
    ...
    public static RuntimeHealthSnapshot From(BotRuntimeState state, TradingBot.Options.TradingBotOptions? options)
    {
        ...
        return new RuntimeHealthSnapshot(
            ...
            PaperDiagnosticsLimitedEligible: state.OrderBookServiceStats.OrderbookCircuitBreakerState == "Closed" && state.OrderBookServiceStats.TruePostBreakerBadRequests == 0 && state.OrderBookServiceStats.MarketOrderbookQuarantineActive == 0 && state.OrderBookServiceStats.InvalidTokenQuarantineActive == 0 && state.DiagnosticsCounterMismatchCount == 0 && state.LiveTradingBlockedCount == 0 && TradingBot.Services.LiveTradingGuard.SigningAttempts == 0,
            PaperDiagnosticsLimitedBlockedReason: PaperDiagnosticsLimitedReason(state),
            OrderbookStableNow: state.OrderBookServiceStats.OrderbookCircuitBreakerState == "Closed" && state.OrderBookServiceStats.TruePostBreakerBadRequests == 0 && state.OrderBookServiceStats.MarketOrderbookQuarantineActive == 0 && state.OrderBookServiceStats.InvalidTokenQuarantineActive == 0,
            ...
            ReducedUniverseOrderbookStableNow: state.OrderBookServiceStats.OrderbookCircuitBreakerState == "Closed" && state.OrderBookServiceStats.TruePostBreakerBadRequests == 0 && state.OrderBookServiceStats.MarketOrderbookQuarantineActive == 0 && state.OrderBookServiceStats.InvalidTokenQuarantineActive == 0,
    ...
    private static string PaperDiagnosticsLimitedReason(BotRuntimeState state)
    {
        ...
        if (stats.MarketOrderbookQuarantineActive > 0) reasons.Add("MarketOrderbookQuarantineActive");
        if (!stats.MarketOrderbookQuarantineLifecycleBalanced || !stats.InvalidTokenQuarantineLifecycleBalanced) reasons.Add("QuarantineLifecycleUnbalanced");
        if (state.DiagnosticsCounterMismatchCount > 0) reasons.Add("DiagnosticsCounterMismatch");
        if (state.LiveTradingBlockedCount > 0) reasons.Add("LiveTradingBlocked");
...
public static class RuntimeHealthTrendTracker
{
    ...
    public static string ToSoakStatusLogLine(RuntimeHealthSnapshot health, RuntimeHealthTrend trend, TradingBot.Options.TradingBotOptions? options = null, BotRuntimeState? state = null)
    {
        ...
        var paperDiagnosticsLimitedEligibleNow = orderbookStableNow
            && trend.BatchBookBadRequestsDeltaLastHour == 0
            && trend.BatchBookInvalidTokensDeltaLastHour == 0
            && health.TruePostBreakerBadRequests == 0
        ...
        var paperDiagnosticsLimitedBlockedReasonNow = paperDiagnosticsLimitedEligibleNow ? "None" : string.Join("|", new[]
        {
            ...
            trend.BatchBookBadRequestsDeltaLastHour == 0 ? "" : "BatchBookBadRequestsDeltaWindow",
            trend.BatchBookInvalidTokensDeltaLastHour == 0 ? "" : "BatchBookInvalidTokensDeltaWindow",
            ...
            memoryStable ? "" : "MemoryStableFalse",
            logVolumeStable ? "" : "LogVolumeStableFalse",
            health.LiveTradingBlockedCount == 0 ? "" : "LiveTradingBlocked",
            health.SigningAttempts == 0 ? "" : "SigningAttempts"
