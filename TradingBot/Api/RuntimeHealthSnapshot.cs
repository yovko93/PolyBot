...
public static class RuntimeHealthTrendTracker
{
    ...
    public static string ToSoakStatusLogLine(RuntimeHealthSnapshot health, RuntimeHealthTrend trend, TradingBot.Options.TradingBotOptions? options = null, BotRuntimeState? state = null)
    {
        ...
        var strategyStatus = string.Join(",", health.StrategyCounters.OrderBy(x => x.Key, StringComparer.OrdinalIgnoreCase).Select(x => $"{x.Key}:{x.Value.Mode}:scan={x.Value.Scanned}:paper={x.Value.PaperOpened}:blocked={x.Value.DiagnosticsOnlyBlocked}:faults={x.Value.Faults}"));
