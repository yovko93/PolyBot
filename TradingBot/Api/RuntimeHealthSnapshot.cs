...
public sealed record RuntimeHealthSnapshot(
    ...
    string PaperCounterSourceOfTruth,
    int PaperCounterDuplicateSuppressions,
    string PaperCounterLastDuplicateExecutionId,
    int PaperCounterCountedExecutionIds,
    int PaperCounterSyntheticCanaryCountedExecutions,
    int PaperCounterRealScannerCountedExecutions,
    bool PaperCounterAuditEnabled,
    int PaperCounterIncrementLogsWritten,
    int PaperCounterDuplicateIncrementLogSuppressions,
    int PaperCounterDuplicateSuppressionLogsWritten,
    string PaperCounterLastIncrementExecutionId,
    string PaperCounterLastSuppressedDuplicateExecutionId,
    bool PaperCounterAuditConsistent,
    ...
    IReadOnlyDictionary<string, TradingBot.Services.StrategyRuntimeCounterSnapshot> StrategyCounters)
{
    ...
    private TradingBot.Services.PaperPhase1PositiveCaptureState PositiveCapture => TradingBot.Services.PaperPhase1PositiveCaptureService.Current;
    private TradingBot.Services.PaperPhase1RealReadinessState RealReadiness => TradingBot.Services.PaperPhase1RealReadinessMonitor.Current;
    private TradingBot.Services.PaperPhase1ContractFixtureState ContractFixture => TradingBot.Services.PaperPhase1ContractFixtureService.Current;
    public bool PaperPhase1ContractFixtureEnabled => ContractFixture.Enabled;
    public bool PaperPhase1ContractFixtureDryReplayOnly => ContractFixture.DryReplayOnly;
    public bool PaperPhase1ContractFixtureAllowPaperOpen => ContractFixture.AllowPaperOpen;
    public bool PaperPhase1ContractFixtureOpened => ContractFixture.Opened;
    public bool PaperPhase1ContractFixtureConsistent => ContractFixture.Consistent;
    public string PaperPhase1ContractFixtureConsistencyReason => ContractFixture.ConsistencyReason;
    public int PaperPhase1ContractFixtureAlertLevel => ContractFixture.AlertLevel;
    public string PaperPhase1ContractFixtureStageReached => ContractFixture.Opened ? "Opened" : ContractFixture.PaperEligible ? "PaperEligible" : ContractFixture.CleanPositive ? "CleanPositive" : "None";
    public bool PaperPhase1ContractFixtureOpenAttempted => ContractFixture.OpenAttempted;
    public string PaperPhase1ContractFixturePositionId => ContractFixture.PositionId;
    public bool PaperPhase1ContractFixtureSettled => ContractFixture.SettlementSucceeded;
    public bool PaperPhase1ContractFixtureCandidateInjected => ContractFixture.Enabled && (ContractFixture.CleanPositive || ContractFixture.PaperEligible);
    public bool PaperPhase1ContractFixtureAffectsRuntime => ContractFixture.Enabled && ContractFixture.Position?.CountsTowardNormalRuntime == true;
    public bool PaperPhase1ContractFixtureIsolationOk => !PaperPhase1ContractFixtureAffectsRuntime
        && (!PaperPhase1ContractFixtureCandidateInjected || PaperPhase1ContractFixtureEnabled);
    public string PaperPhase1ContractFixtureIsolationReason => PaperPhase1ContractFixtureIsolationOk ? "None" : "FixtureStateAffectsNormalRuntime";
    private TradingBot.Services.PaperPhase1ReleaseStatus ReleaseStatus => TradingBot.Services.PaperPhase1ReleaseStatusService.Current;
    public bool PaperPhase1ReleaseStatusEnabled => ReleaseStatus.ReleaseStatusEnabled;
    public int PaperPhase1ReleaseStatusLogIntervalSeconds => ReleaseStatus.ReleaseStatusLogIntervalSeconds;
    public long PaperPhase1ReleaseStatusLogsWritten => ReleaseStatus.ReleaseStatusLogsWritten;
    public long PaperPhase1ReleaseStatusLogsSuppressed => ReleaseStatus.ReleaseStatusLogsSuppressed;
    public DateTime? PaperPhase1ReleaseStatusLastEmittedUtc => ReleaseStatus.ReleaseStatusLastEmittedUtc;
    public string PaperPhase1ReleaseStatusLastChangeReason => ReleaseStatus.ReleaseStatusLastChangeReason;
    public bool PaperPhase1ReleaseStatusConsistent => ReleaseStatus.ReleaseStatusConsistent;
    public bool PaperPhase1ReleaseStatusExportWritten => ReleaseStatus.ReleaseStatusExportWritten;
    public string PaperPhase1ReleaseStatusExportPath => ReleaseStatus.ReleaseStatusExportPath;
    public string PaperPhase1ReleaseStatusLastWriteError => ReleaseStatus.ReleaseStatusLastWriteError;
    public bool PaperPhase1OperatorRunbookEnabled => ReleaseStatus.OperatorRunbookEnabled;
    public int PaperPhase1OperatorRunbookIntervalSeconds => ReleaseStatus.OperatorRunbookIntervalSeconds;
    public long PaperPhase1OperatorRunbookLogsWritten => ReleaseStatus.OperatorRunbookLogsWritten;
    public long PaperPhase1OperatorRunbookLogsSuppressed => ReleaseStatus.OperatorRunbookLogsSuppressed;
    public DateTime? PaperPhase1OperatorRunbookLastEmittedUtc => ReleaseStatus.OperatorRunbookLastEmittedUtc;
    public string PaperPhase1OperatorRunbookLastReason => ReleaseStatus.OperatorRunbookLastReason;
    public bool PaperPhase1OperatorRunbookConsistent => ReleaseStatus.OperatorRunbookConsistent;
    public string PaperPhase1OperatorRunbookConsistencyReason => ReleaseStatus.OperatorRunbookConsistencyReason;
    public bool PaperPhase1OperatorRunbookExportWritten => ReleaseStatus.OperatorRunbookExportWritten;
    public string PaperPhase1OperatorRunbookExportPath => ReleaseStatus.OperatorRunbookExportPath;
    public string PaperPhase1OperatorRunbookLastWriteError => ReleaseStatus.OperatorRunbookLastWriteError;
    public int PaperPhase1NormalRuntimeOpened => Math.Max(0, RealWatch.OpenSucceeded);
    public int PaperPhase1NormalRuntimeClosed => Math.Max(0, RealWatch.ClosedPositions);
    public int PaperPhase1NormalRuntimeOpenPositions => Math.Max(0, RealWatch.OpenPositions);
    public int PaperPhase1FixtureOpened => ContractFixture.Enabled ? ContractFixture.PaperOpened : 0;
    public int PaperPhase1FixtureClosed => ContractFixture.Enabled ? ContractFixture.PaperClosed : 0;
    public int PaperPhase1FixtureOpenPositions => ContractFixture.Enabled ? ContractFixture.PaperOpenPositions : 0;
    public int PaperPhase1CanaryClosed => PaperPhase1CanaryPaperClosed;
    public bool PaperPhase1RealSettlementEnabled => PaperPhase1ProfileActive;
    public bool PaperPhase1RealSettlementRequested => ContractFixture.SettlementRequested;
    public bool PaperPhase1RealSettlementSucceeded => ContractFixture.SettlementSucceeded;
    public bool PaperPhase1RealSettlementFailed => ContractFixture.SettlementFailed;
    public bool PaperPhase1RealSettlementRejected => ContractFixture.SettlementRejected;
    public string PaperPhase1RealSettlementLastPositionId => ContractFixture.SettlementLastPositionId;
    public string PaperPhase1RealSettlementLastReason => ContractFixture.SettlementLastReason;
    public decimal? PaperPhase1RealSettlementLastRealizedPayout => ContractFixture.SettlementLastRealizedPayout;
    public decimal? PaperPhase1RealSettlementLastRealizedPnl => ContractFixture.SettlementLastRealizedPnl;
    public bool PaperPhase1RealSettlementConsistent => ContractFixture.SettlementConsistent;
    public string PaperPhase1RealSettlementConsistencyReason => ContractFixture.SettlementConsistencyReason;
    public bool PaperPhase1LifecycleBalanceOk => ContractFixture.LifecycleBalanceOk;
    public string PaperPhase1LifecycleBalanceReason => ContractFixture.LifecycleBalanceReason;
    public bool PaperPhase1RealSoakEnabled => RealReadiness.Enabled;
    public DateTime PaperPhase1RealSoakStartedUtc => RealReadiness.StartedUtc;
    public TimeSpan PaperPhase1RealSoakUptime => RealReadiness.Uptime;
    public bool PaperPhase1RealSoakProfileActive => RealReadiness.ProfileActive;
    public bool PaperPhase1RealSoakReadinessStable => RealReadiness.ReadinessStable;
    public double PaperPhase1RealSoakReadinessStableMinutes => RealReadiness.ReadinessStableMinutes;
    public double PaperPhase1RealSoakOrderbookStableMinutes => RealReadiness.OrderbookStableMinutes;
    public bool PaperPhase1RealSoakMemoryStable => RealReadiness.MemoryStable;
    public bool PaperPhase1RealSoakLogVolumeStable => RealReadiness.LogVolumeStable;
    public bool PaperPhase1RealSoakNoCounterMismatches => RealReadiness.NoCounterMismatches;
    public bool PaperPhase1RealSoakNoUnexpectedPaperOpens => RealReadiness.NoUnexpectedPaperOpens;
    public bool PaperPhase1RealSoakNoSigningAttempts => RealReadiness.NoSigningAttempts;
    public bool PaperPhase1RealSoakNoLiveTradingBlocks => RealReadiness.NoLiveTradingBlocks;
    public bool PaperPhase1RealSoakConsistent => RealReadiness.Consistent;
    public string PaperPhase1RealSoakConsistencyReason => RealReadiness.ConsistencyReason;
    public int PaperPhase1RealAlertLevel => RealReadiness.AlertLevel;
    public string PaperPhase1RealAlertName => RealReadiness.AlertName;
    public string PaperPhase1RealAlertReason => RealReadiness.AlertReason;
    public string PaperPhase1RealAlertCandidateId => RealReadiness.CandidateId;
    public string PaperPhase1RealAlertMarketId => RealReadiness.MarketId;
    public decimal? PaperPhase1RealAlertAfterSafetyEdge => RealReadiness.AfterSafetyEdge;
    public decimal? PaperPhase1RealAlertDistanceToMinEdge => RealReadiness.DistanceToMinEdge;
    public string PaperPhase1RealAlertFirstBlockingReason => RealReadiness.FirstBlockingReason;
    public DateTime PaperPhase1RealAlertLastChangedUtc => RealReadiness.LastChangedUtc;
    public int PaperPhase1CleanNearOpenCount => RealReadiness.CleanNearOpenCount;
    public decimal PaperPhase1CleanNearOpenThresholdDistance => RealReadiness.CleanNearOpenThresholdDistance;
    public string PaperPhase1CleanNearOpenBestCandidateId => RealReadiness.CleanNearOpenBestCandidateId;
    public decimal? PaperPhase1CleanNearOpenBestAfterSafetyEdge => RealReadiness.CleanNearOpenBestAfterSafetyEdge;
    public decimal? PaperPhase1CleanNearOpenBestDistanceToMinEdge => RealReadiness.CleanNearOpenBestDistanceToMinEdge;
    public string PaperPhase1CleanNearOpenFirstBlockingReason => RealReadiness.CleanNearOpenFirstBlockingReason;
    public string PaperPhase1CleanNearOpenAllBlockingReasons => RealReadiness.CleanNearOpenAllBlockingReasons;
    public bool PaperPhase1PositiveCaptureEnabled => PositiveCapture.Enabled;
    public decimal PaperPhase1PositiveCaptureThreshold => PositiveCapture.Threshold;
    public int PaperPhase1PositiveCapturesTotal => PositiveCapture.CapturesTotal;
    public int PaperPhase1PositiveCapturesAboveMinEdge => PositiveCapture.CapturesAboveMinEdge;
    public int PaperPhase1PositiveCapturesPaperEligible => PositiveCapture.CapturesPaperEligible;
    public int PaperPhase1PositiveCapturesOpened => PositiveCapture.CapturesOpened;
    public decimal? PaperPhase1PositiveBestCapturedAfterSafetyEdge => PositiveCapture.BestAfterSafetyEdge;
    public string PaperPhase1PositiveBestCapturedCandidateId => PositiveCapture.BestCandidateId;
    public string PaperPhase1PositiveBestCapturedFirstBlockingReason => PositiveCapture.BestFirstBlockingReason;
    public bool PaperPhase1PositiveCaptureConsistent => PositiveCapture.Consistent && (PaperCounterRealScannerCountedExecutions == 0 || PositiveCapture.CapturesOpened == PaperCounterRealScannerCountedExecutions);
    public bool PaperPhase1PositiveCaptureExportWritten => PositiveCapture.ExportWritten;
    public string PaperPhase1PositiveCaptureLastWriteError => PositiveCapture.LastWriteError;
    public int PaperPhase1ValidPositiveCapturesTotal => PositiveCapture.ValidCapturesTotal;
    public int PaperPhase1ValidPositiveCapturesAboveMinEdge => PositiveCapture.ValidAboveMinEdge;
    public int PaperPhase1ValidPositiveCapturesPaperEligible => PositiveCapture.ValidPaperEligible;
    public int PaperPhase1ValidPositiveCapturesOpened => PositiveCapture.ValidOpened;
    public decimal? PaperPhase1ValidPositiveBestAfterSafetyEdge => PositiveCapture.BestValidAfterSafetyEdge;
    public string PaperPhase1ValidPositiveBestCandidateId => PositiveCapture.BestValidCandidateId;
    public string PaperPhase1ValidPositiveBestFirstBlockingReason => PositiveCapture.BestValidFirstBlockingReason;
    public int PaperPhase1InvalidPositiveArtifactsTotal => PositiveCapture.InvalidArtifactsTotal;
    public int PaperPhase1InvalidPositiveArtifactsAboveMinEdge => PositiveCapture.InvalidArtifactsAboveMinEdge;
    public string PaperPhase1InvalidPositiveArtifactsByReason => string.Join("|", PositiveCapture.InvalidArtifactsByReason.Select(x=>$"{x.Key}={x.Value}"));
    public decimal? PaperPhase1InvalidPositiveArtifactBestAfterSafetyEdge => PositiveCapture.BestInvalidArtifactAfterSafetyEdge;
    public string PaperPhase1InvalidPositiveArtifactBestCandidateId => PositiveCapture.BestInvalidArtifactCandidateId;
    public string PaperPhase1InvalidPositiveArtifactBestFirstReason => PositiveCapture.BestInvalidArtifactFirstReason;
    public int PaperPhase1PositiveInvalidArtifactsTotal => PositiveCapture.InvalidArtifactsTotal;
    public int PaperPhase1PositiveCleanCapturesTotal => PositiveCapture.ValidCapturesTotal;
    public int PaperPhase1PositiveCleanAboveMinEdge => PositiveCapture.ValidAboveMinEdge;
    public decimal? PaperPhase1PositiveCleanBestAfterSafetyEdge => PositiveCapture.BestValidAfterSafetyEdge;
    public string PaperPhase1PositiveCleanBestCandidateId => PositiveCapture.BestValidCandidateId;
    public string PaperPhase1PositiveCleanBestFirstBlockingReason => PositiveCapture.BestValidFirstBlockingReason;
    public int PaperPhase1PositiveRealWatchTotal => PositiveCapture.RealWatchTotal;
    public int PaperPhase1PositiveRealWatchAboveMinEdge => PositiveCapture.RealWatchAboveMinEdge;
    public decimal? PaperPhase1PositiveRealWatchBestAfterSafetyEdge => PositiveCapture.BestRealWatchAfterSafetyEdge;
    public string PaperPhase1PositiveRealWatchBestCandidateId => PositiveCapture.BestRealWatchCandidateId;
    public int PaperPhase1PositiveExecutableLikeTotal => PositiveCapture.ExecutableLikeTotal;
    public decimal? PaperPhase1PositiveExecutableLikeBestAfterSafetyEdge => PositiveCapture.BestExecutableLikeAfterSafetyEdge;
    public int PaperPhase1PositivePaperEligibleTotal => PositiveCapture.PaperEligibleTotal;
    public int PaperPhase1PositiveOpenedTotal => PositiveCapture.OpenedTotal;
    public int PaperPhase1CleanPositiveExcludedFromRealWatchTotal => PositiveCapture.CleanExcludedFromRealWatchTotal;
    public string PaperPhase1CleanPositiveExcludedFromRealWatchByReason => string.Join("|", PositiveCapture.CleanExcludedFromRealWatchByReason.Select(x=>$"{x.Key}={x.Value}"));
    public string PaperPhase1BestCleanPositiveExcludedFromRealWatchCandidateId => PositiveCapture.BestCleanExcludedCandidateId;
    public decimal? PaperPhase1BestCleanPositiveExcludedFromRealWatchAfterSafetyEdge => PositiveCapture.BestCleanExcludedAfterSafetyEdge;
    public string PaperPhase1BestCleanPositiveExcludedFromRealWatchReason => PositiveCapture.BestCleanExcludedReason;
    public string PaperPhase1BestCleanPositiveExcludedFromRealWatchAllReasons => PositiveCapture.BestCleanExcludedAllReasons;
    public decimal? BestValidCleanAfterSafetyEdge => PositiveCapture.BestValidAfterSafetyEdge;
    ...
    public string PaperPhase1StrategyBestCandidateId => PositiveReconciliation.StrategyBestCandidateId;
    public string PaperPhase1StrategyBestCandidateReason => PositiveReconciliation.StrategyBestCandidateReason;
    public long PaperPhase1RealWatchPositiveAfterSafety => PositiveReconciliation.RealWatchPositiveAfterSafety;
    public bool PaperPhase1PositiveCandidateMismatch => PositiveReconciliation.Mismatch;
    public string PaperPhase1PositiveCandidateMismatchReason => PositiveReconciliation.MismatchReason;
    public bool PaperPhase1PositiveCandidateMismatchBlocking => PositiveReconciliation.MismatchBlocking;
    public bool PaperPhase1PositiveReconciliationConsistent => PositiveReconciliation.Consistent;
    public long PaperPhase1PositiveExcludedTotal => PositiveReconciliation.PositiveExcludedTotal;
    public string PaperPhase1PositiveExcludedByReason => string.Join("|", PositiveReconciliation.ExcludedByReason.Select(x=>$"{x.Key}={x.Value}"));
    public string PaperPhase1PositiveBestExcludedCandidateId => PositiveReconciliation.BestExcludedCandidateId;
    public string PaperPhase1PositiveBestExcludedMarketId => PositiveReconciliation.BestExcludedMarketId;
    public decimal? PaperPhase1PositiveBestExcludedAfterSafetyEdge => PositiveReconciliation.BestExcludedAfterSafetyEdge;
    public string PaperPhase1PositiveBestExcludedReason => PositiveReconciliation.BestExcludedReason;
    public string PaperPhase1PositiveBestExcludedAllReasons => PositiveReconciliation.BestExcludedAllReasons;
    public long RealPositiveAfterSafetyRaw => PositiveReconciliation.RealPositiveAfterSafetyRaw;
    public long RealPositiveAfterSafetyValid => PositiveCapture.RealWatchTotal;
    public long RealPositiveAfterSafetyRealWatch => PositiveCapture.RealWatchTotal;
    public long RealPositiveAfterSafetyExecutableLike => PositiveReconciliation.RealPositiveAfterSafetyExecutableLike;
    public long RealPositiveAfterSafetyExcluded => PositiveReconciliation.RealPositiveAfterSafetyExcluded;
    public string RealPositiveExcludedByReason => PaperPhase1PositiveExcludedByReason;
    public decimal? RealBestPositiveAfterSafetyEdge => PositiveReconciliation.BestExcludedAfterSafetyEdge ?? PositiveReconciliation.StrategyBestAfterSafetyEdge;
    public string RealBestPositiveCandidateId => PositiveReconciliation.BestExcludedCandidateId;
    public string RealBestPositiveFirstBlockingReason => PositiveReconciliation.BestExcludedReason;
    public string RealBestPositiveAllBlockingReasons => PositiveReconciliation.BestExcludedAllReasons;
    public decimal? BestPositiveAfterSafetyEdge => PositiveReconciliation.StrategyBestAfterSafetyEdge;
    public string BestPositiveCandidateId => PositiveReconciliation.StrategyBestCandidateId;
    public string BestPositiveCandidateValidity => PositiveReconciliation.RealPositiveAfterSafetyValid > 0 ? "Valid" : "Excluded";
    public string BestPositiveCandidateFirstBlockingReason => PositiveReconciliation.BestExcludedReason;
    public string BestPositiveCandidateAllBlockingReasons => PositiveReconciliation.BestExcludedAllReasons;
    public decimal? BestExecutableLikeAfterSafetyEdge => PositiveReconciliation.RealPositiveAfterSafetyExecutableLike > 0 ? PositiveReconciliation.StrategyBestAfterSafetyEdge : null;
    public string BestExecutableLikeCandidateId => PositiveReconciliation.RealPositiveAfterSafetyExecutableLike > 0 ? PositiveReconciliation.StrategyBestCandidateId : "None";
    public string BestExecutableLikeFirstBlockingReason => PositiveReconciliation.RealPositiveAfterSafetyExecutableLike > 0 ? "None" : PositiveReconciliation.BestExcludedReason;
    public bool PaperPhase1RealWatchEnabled => RealWatch.Enabled;
    public bool PaperPhase1RealWatchProfileActive => RealWatch.ProfileActive;
    public bool PaperPhase1RealWatchArmed => RealWatch.Armed;
    public bool PaperPhase1RealWatchWaitingForEdge => RealWatch.WaitingForEdge;
    public decimal? PaperPhase1RealWatchBestAfterSafetyEdge => RealWatch.BestAfterSafetyEdge;
    public decimal? PaperPhase1RealWatchBestDistanceToMinEdge => RealWatch.BestDistanceToMinEdge;
    public string PaperPhase1RealWatchBestCandidateId => RealWatch.BestCandidateId;
    public string PaperPhase1RealWatchBestMarketId => RealWatch.BestMarketId;
    public string PaperPhase1RealWatchTopBlockingReason => RealWatch.TopBlockingReason;
    public string PaperPhase1RealWatchLastEligibleCandidateId => RealWatch.LastEligibleCandidateId;
    public DateTime? PaperPhase1RealWatchLastOpenAttemptUtc => RealWatch.LastOpenAttemptUtc;
    public string PaperPhase1RealWatchLastOpenResult => RealWatch.LastOpenResult;
    public string PaperPhase1RealWatchLastOpenBlockedReason => RealWatch.LastOpenBlockedReason;
    public int PaperPhase1RealWatchOpenAttempts => RealWatch.OpenAttempts;
    public int PaperPhase1RealWatchOpenSucceeded => RealWatch.OpenSucceeded;
    public int PaperPhase1RealWatchOpenFailed => RealWatch.OpenFailed;
    public string PaperPhase1RealWatchOpenedPositionId => RealWatch.OpenedPositionId;
    public int PaperPhase1RealWatchOpenPositions => RealWatch.OpenPositions;
    public decimal PaperPhase1RealWatchExposure => RealWatch.Exposure;
    public decimal PaperPhase1RealWatchRealizedPnl => RealWatch.RealizedPnl;
    public bool PaperPhase1RealWatchConsistent => RealWatch.Consistent;
    public string PaperPhase1RealWatchConsistencyReason => RealWatch.ConsistencyReason;
    public int PaperPhase1RealOpenPositionCount => RealWatch.OpenPositions;
    public int PaperPhase1RealClosedPositionCount => RealWatch.ClosedPositions;
    public decimal PaperPhase1RealExposure => RealWatch.Exposure;
    public decimal PaperPhase1RealLocked => RealWatch.Locked;
    public decimal PaperPhase1RealRealizedPnl => RealWatch.RealizedPnl;
    public string PaperPhase1RealLastOpenedPositionId => RealWatch.OpenedPositionId;
    public string PaperPhase1RealLastClosedPositionId => RealWatch.LastClosedPositionId;
    public string PaperPhase1RealLastSettlementReason => RealWatch.LastSettlementReason;
    public bool PaperPhase1RealLifecycleConsistent => RealWatch.Consistent;
    public string PaperPhase1RealLifecycleConsistencyReason => RealWatch.ConsistencyReason;
    public long FormulaWarningsTotal => TradingBot.Services.FormulaDiagnostics.Current.WarningsTotal;
    public long FormulaWarningsBlocking => TradingBot.Services.FormulaDiagnostics.Current.WarningsBlocking;
    public long FormulaWarningsNonBlocking => TradingBot.Services.FormulaDiagnostics.Current.WarningsNonBlocking;
    public long FormulaWarningsSuppressed => TradingBot.Services.FormulaDiagnostics.Current.WarningsSuppressed;
    public string FormulaLastWarning => TradingBot.Services.FormulaDiagnostics.Current.LastWarning.Replace(' ', '_');
    public string FormulaLastWarningClassification => TradingBot.Services.FormulaDiagnostics.Current.LastWarningClassification;
    public bool FormulaDiagnosticsConsistent => TradingBot.Services.FormulaDiagnostics.Current.Consistent;
    ...
    public bool DiagnosticsDashboardHistoryLastWriteOk => TradingBot.Services.DiagnosticsDashboardHistoryService.DiagnosticsDashboardHistoryLastWriteOk;
    public string DiagnosticsDashboardHistoryLastWriteError => TradingBot.Services.DiagnosticsDashboardHistoryService.DiagnosticsDashboardHistoryLastWriteError;

    public bool ExpectedReducedUniverseGlobalBlock => PaperPhase1ProfileActive
        ...
        && PaperPhase1ReducedUniversePaperExplicitlyAllowed
        && !TradingReadiness
        && (string.Equals(SoakReadiness, "Blocked", StringComparison.OrdinalIgnoreCase) || string.Equals(SoakReadiness, "BlockedReducedUniverseDiagnosticsOnly", StringComparison.OrdinalIgnoreCase));
    public bool DiagnosticsCounterMismatchExpectedReducedUniverseGlobalBlock => ExpectedReducedUniverseGlobalBlock && DiagnosticsCounterMismatchLastReason.Contains("UnhealthyDiscoveryRequiresUnstableRuntime", StringComparison.OrdinalIgnoreCase);
    public long DiagnosticsCounterMismatchBlockingCount => DiagnosticsCounterMismatchPaperBlocking ? DiagnosticsCounterMismatchCount : 0;
    public long DiagnosticsCounterMismatchNonBlockingCount => DiagnosticsCounterMismatchPaperBlocking ? 0 : DiagnosticsCounterMismatchCount;
    public string DiagnosticsCounterMismatchLastBlockingReason => DiagnosticsCounterMismatchPaperBlocking ? DiagnosticsCounterMismatchLastReason : "None";
    public string DiagnosticsCounterMismatchLastNonBlockingReason => DiagnosticsCounterMismatchPaperBlocking ? "None" : DiagnosticsCounterMismatchCategory;
    public bool DiagnosticsCounterMismatchPaperBlocking => DiagnosticsCounterMismatchCount > 0 && !DiagnosticsCounterMismatchExpectedReducedUniverseGlobalBlock;
    public string DiagnosticsCounterMismatchCategory => DiagnosticsCounterMismatchCount <= 0 ? "None" : DiagnosticsCounterMismatchExpectedReducedUniverseGlobalBlock ? "ExpectedReducedUniverseGlobalBlock" : DiagnosticsCounterMismatchLastReason.Contains("ReadinessInvariant", StringComparison.OrdinalIgnoreCase) ? "GlobalReadinessInvariantMismatch" : DiagnosticsCounterMismatchLastReason.Contains("Paper", StringComparison.OrdinalIgnoreCase) ? "PaperSafetyInvariantMismatch" : DiagnosticsCounterMismatchLastReason.Contains("Skew", StringComparison.OrdinalIgnoreCase) ? "SnapshotSkew" : "CounterArithmeticMismatch";
    public bool GlobalTradingReadiness => TradingReadiness;
    public string GlobalTradingReadinessReason => TradingReadiness ? "Ready" : string.Equals(DiscoverySelectedSource, "ReducedUniverseDiagnosticsOnly", StringComparison.OrdinalIgnoreCase) ? "ReducedUniverseDiagnosticsOnly" : (DiscoveryBlockedReason == "None" ? SoakReadinessReason : DiscoveryBlockedReason);
    public string GlobalSoakReadiness => SoakReadiness;
    public string GlobalSoakReadinessReason => SoakReadinessReason;
    public bool LocalPaperPhase1Readiness => PaperPhase1Readiness;
    public string LocalPaperPhase1ReadinessReason => PaperPhase1ReadinessReason;
    public bool DiscoveryInvariantAppliesToGlobalReadiness => true;
    public bool DiscoveryInvariantAppliesToPaperPhase1 => !ExpectedReducedUniverseGlobalBlock;
    public string DiscoveryInvariantScope => DiscoveryInvariantAppliesToPaperPhase1 ? "GlobalAndPaperPhase1" : "GlobalOnly";

    public bool PaperPhase1ProfileActive => string.Equals(RuntimeProfile, TradingBot.Services.RuntimeProfileService.ReducedDiagnosticsPaperPhase1, StringComparison.OrdinalIgnoreCase) || string.Equals(RuntimeProfile, TradingBot.Services.RuntimeProfileService.ReducedDiagnosticsPaperPhase1Canary, StringComparison.OrdinalIgnoreCase);
    ...
    public bool PaperPhase1ReducedUniversePaperExplicitlyAllowed => PaperPhase1ProfileActive && AllowReducedUniverseDiagnosticsOnly && ReducedUniverseRequireExplicitFlag && ReducedUniverseExplicitFlagSatisfied && PaperBlockedByDiscoveryMode == 0;
    ...
    public DateTime PaperPhase1ReadinessLastEvaluatedUtc => DateTime.UtcNow;
    public long PaperPhase1ReadinessEvaluationCount => Math.Max(0, ScannerCycleId);
    public bool PaperPhase1ReadinessUsedCurrentWarmupComplete => Uptime > TimeSpan.Zero;
    public bool PaperPhase1ReadinessUsedCurrentMemoryStable => !LongRunBlockingReason.Contains("Memory", StringComparison.OrdinalIgnoreCase);
    public bool PaperPhase1ReadinessUsedCurrentLogVolumeStable => RuntimeStatusExportStable;
    public bool PaperPhase1ReadinessUsedCurrentReducedUniversePaperAllowed => PaperPhase1ReducedUniversePaperExplicitlyAllowed;
    public int PaperPhase1ReadinessStaleReasonClearedCount => PaperPhase1ReadinessUsedCurrentMemoryStable && !PaperPhase1ReadinessCurrentBlockingReasons.Contains("MemoryNotStable", StringComparison.OrdinalIgnoreCase) ? 1 : 0;
    public string PaperPhase1ReadinessCurrentBlockingReasons
    {
        get
        {
            var reasons = new List<string>();
            if (!PaperPhase1ProfileActive) reasons.Add("ProfileNotActive");
            if (!PaperPhase1Enabled) reasons.Add("PaperPhase1Disabled");
            if (!PaperPhase1LiveTradingDisabled) reasons.Add("LiveTradingNotDisabled");
            if (!PaperPhase1SigningDisabled) reasons.Add("SigningAttemptsDetected");
            if (!PaperDiagnosticsLimitedEligible) reasons.Add(string.IsNullOrWhiteSpace(PaperDiagnosticsLimitedBlockedReason) ? "PaperDiagnosticsLimitedNotEligible" : PaperDiagnosticsLimitedBlockedReason);
            if (!string.Equals(PaperDiagnosticsLimitedAllowedStrategy, "SingleMarketBuyBoth", StringComparison.OrdinalIgnoreCase)) reasons.Add("AllowedStrategyMismatch");
            if (!PaperEligibleStrategies.Split('|', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries).SequenceEqual(new[] { "SingleMarketBuyBoth" }, StringComparer.OrdinalIgnoreCase)) reasons.Add("PaperEligibleStrategyMismatch");
            if (!OrderbookStableNow) reasons.Add("OrderbookUnstable");
            if (!ReducedUniverseOrderbookStableNow) reasons.Add("ReducedUniverseOrderbookUnstable");
            if (!PaperPhase1ReadinessUsedCurrentWarmupComplete) reasons.Add("WarmupNotComplete");
            if (!PaperPhase1ReadinessUsedCurrentMemoryStable) reasons.Add("MemoryNotStable");
            if (!PaperPhase1ReadinessUsedCurrentLogVolumeStable) reasons.Add("LogVolumeNotStable");
            if (PaperDiagnosticsLimitedMaxOpenPositions != 1 || PaperDiagnosticsLimitedMaxPaperNotionalPerTrade != 5m || PaperDiagnosticsLimitedMaxPaperTotalExposure != 5m || PaperPhase1MaxOpensPerHour != 1 || PaperPhase1MinEdge != 0.01m) reasons.Add("SafeLimitsMismatch");
            if (!PaperPhase1ReadinessUsedCurrentReducedUniversePaperAllowed) reasons.Add("ReducedUniversePaperNotExplicitlyAllowed");
            if (ReducedUniverseMarkets <= 0) reasons.Add("ReducedUniverseEmpty");
            if (ReducedUniverseOrderbookEligibleMarkets <= 0) reasons.Add("ReducedUniverseOrderbookEmpty");
            if (PaperPhase1UsingPersistedSnapshot && PaperPhase1DiscoveryFallbackSnapshotMarkets <= 0) reasons.Add("PersistedSnapshotEmpty");
    ...
    public bool PaperPhase1CanaryRunOnce => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.RunOnce;
    public bool PaperPhase1CanaryAttempted => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.Attempted;
    public bool PaperPhase1CanaryOpened => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.Opened;
    public bool PaperPhase1CanarySettled => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.Settled;
    public bool PaperPhase1CanaryRejected => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.Rejected;
    public string PaperPhase1CanaryRejectedReason => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.RejectedReason;
    public string PaperPhase1CanaryPositionId => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.PositionId;
    public decimal PaperPhase1CanaryExpectedProfit => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.ExpectedProfit;
    public decimal PaperPhase1CanaryRealizedPnl => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.RealizedPnl;
    public int PaperPhase1CanaryDuplicateSuppressions => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.DuplicateSuppressions;
    public string PaperPhase1CanaryLastDuplicateReason => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.LastDuplicateReason;
    public int PaperPhase1CanaryOpenAttempts => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.OpenAttempts;
    public int PaperPhase1CanaryOpenSucceeded => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.OpenSucceeded;
    public int PaperPhase1CanaryPaperOpened => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.PaperOpened;
    public int PaperPhase1CanaryPaperClosed => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.PaperClosed;
    public int PaperPhase1CanaryOpenPositions => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.OpenPositions;
    public decimal PaperPhase1CanaryExposure => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.Exposure;
    public int PaperPhase1CanaryLadderOpenAttempted => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.CanaryLadderOpenAttempted;
    public int PaperPhase1CanaryLadderOpened => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.CanaryLadderOpened;
    public int PaperPhase1CanaryLadderSettled => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.CanaryLadderSettled;
    public bool PaperPhase1CanarySyntheticOnly => true;
    public bool PaperPhase1CanaryRealOrderSent => false;
    public bool PaperPhase1CanarySigningAttempted => false;
    public bool PaperPhase1CanaryConsistent => TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.Consistent && !PaperPhase1CanaryRealOrderSent && !PaperPhase1CanarySigningAttempted;
    public string PaperPhase1CanaryConsistencyReason => PaperPhase1CanaryConsistent ? "None" : TradingBot.Services.PaperPhase1SyntheticCanaryService.Current.ConsistencyReason;
    ...
    public string PaperPhase1LastOpenBlockedReason => PaperPretradeRejects > 0 && !string.Equals(PaperDiagnosticsLimitedGateLastRejectReason, "None", StringComparison.OrdinalIgnoreCase) ? PaperDiagnosticsLimitedGateLastRejectReason : "None";
    ...
    public string PaperPhase1CandidateCounterScope => "Candidates passed into the Paper Phase 1 paper gate";
    public string PaperPhase1LadderCounterScope => "All SingleMarketBuyBoth candidates considered by the scanner and eligibility ladder";
    public bool PaperPhase1EligibilityLadderEnabled => PaperPhase1ProfileActive && PaperPhase1Enabled;
    private TradingBot.Services.StrategyRuntimeCounterSnapshot? SingleMarketCounter => StrategyCounters.TryGetValue("SingleMarketBuyBoth", out var single) ? single : null;
    public long PaperPhase1LadderSeen => Math.Max(SingleMarketCounter?.Candidates ?? 0, SingleMarketRawCandidates);
    public long PaperPhase1LadderTokenMappingVerified => Math.Max(0, PaperPhase1LadderSeen - SingleMarketDataQualityRejected);
    public long PaperPhase1LadderHasBothBooks => Math.Max(SingleMarketCounter?.BothAsks ?? 0, SingleMarketRawCandidates);
    public long PaperPhase1LadderHasYesAsk => Math.Max(PaperPhase1LadderHasBothBooks, PaperPhase1LadderValidPriced + PaperPhase1LadderMissingNoAsk);
    public long PaperPhase1LadderHasNoAsk => Math.Max(PaperPhase1LadderHasBothBooks, PaperPhase1LadderValidPriced + PaperPhase1LadderMissingYesAsk);
    public long PaperPhase1LadderValidPriced => Math.Max(SingleMarketValidEdgeSamples, SingleMarketCounter?.ValidPriced ?? 0);
    public long PaperPhase1LadderAfterCostComputed => PaperPhase1LadderValidPriced;
    public long PaperPhase1LadderAfterSafetyComputed => PaperPhase1LadderValidPriced;
    public long PaperPhase1LadderNearBreakEven => PaperPhase1LadderBestDistanceToMinEdge.HasValue && PaperPhase1LadderBestDistanceToMinEdge.Value <= 0.015m && (PaperPhase1LadderBestAfterSafetyEdge ?? -999m) < PaperPhase1MinEdge ? Math.Min(PaperPhase1LadderAfterSafetyComputed, Math.Max(1, SingleMarketAfterSafetyEdgeMinus2bpToMinus1bp + SingleMarketAfterSafetyEdgeMinus1bpTo0 + SingleMarketAfterSafetyEdge0To1bp + SingleMarketAfterSafetyEdge1bpTo5bp)) : 0;
    public long PaperPhase1LadderPositiveBeforeCost => SingleMarketPositiveBeforeCost;
    public long PaperPhase1LadderPositiveAfterCost => SingleMarketPositiveAfterCost;
    public long PaperPhase1LadderPositiveAfterSafety => SingleMarketPositiveAfterSafety;
    public long PaperPhase1LadderEdgeStable => SingleMarketEdgeStable;
    public long PaperPhase1LadderDepthSufficient => Math.Max(0, SingleMarketExecutionReady + SingleMarketRejectedByFill + SingleMarketRejectedByRisk);
    public long PaperPhase1LadderFillPassed => Math.Max(0, SingleMarketExecutionReady + SingleMarketRejectedByRisk);
    public long PaperPhase1LadderRiskPassed => Math.Max(SingleMarketExecutionReady, PaperPhase1CandidatesEligible);
    public long PaperPhase1LadderPaperEligible => PaperPhase1CandidatesEligible;
    public long PaperPhase1LadderOpenAttempted => SingleMarketCounter?.PaperOpened ?? 0;
    public long PaperPhase1LadderOpened => SingleMarketCounter?.PaperOpened ?? 0;
    public decimal? PaperPhase1LadderBestAfterSafetyEdge => SingleMarketBestAfterSafetyEdge ?? SingleMarketCounter?.BestEdge;
    public decimal? PaperPhase1LadderBestDistanceToMinEdge => PaperPhase1LadderBestAfterSafetyEdge.HasValue ? Math.Max(0m, PaperPhase1MinEdge - PaperPhase1LadderBestAfterSafetyEdge.Value) : null;
    public decimal? PaperPhase1LadderMedianDistanceToMinEdge => SingleMarketAfterSafetyEdgeP50.HasValue ? Math.Max(0m, PaperPhase1MinEdge - SingleMarketAfterSafetyEdgeP50.Value) : null;
    public string PaperPhase1LadderRejectedByReason => FormatRejectedReasons((SingleMarketCounter?.RejectedByReason ?? new Dictionary<string, long>()).Concat(new[] { new KeyValuePair<string, long>("BelowMinEdge", SingleMarketBelowMinEdge), new KeyValuePair<string, long>("MissingNoAsk", PaperPhase1LadderMissingNoAsk), new KeyValuePair<string, long>("MissingYesAsk", PaperPhase1LadderMissingYesAsk) }).GroupBy(x => TradingBot.Services.PaperPhase1EligibilityLadderExporter.NormalizeReason(x.Key), StringComparer.OrdinalIgnoreCase).ToDictionary(x => x.Key, x => x.Sum(v => v.Value), StringComparer.OrdinalIgnoreCase));
    public string PaperPhase1LadderTopBlockingReason => SingleMarketCounter?.TopSkipReason is { Length: > 0 } r && r != "N/A" ? TradingBot.Services.PaperPhase1EligibilityLadderExporter.NormalizeReason(r) : (SingleMarketBestRejectedReason == "None" ? "None" : TradingBot.Services.PaperPhase1EligibilityLadderExporter.NormalizeReason(SingleMarketBestRejectedReason));
    public long PaperPhase1LadderTopBlockingReasonCount => SingleMarketCounter?.TopSkipCount ?? 0;
    private long PaperPhase1LadderMissingNoAsk => ReasonCount("MissingNoAsk");
    private long PaperPhase1LadderMissingYesAsk => ReasonCount("MissingYesAsk");
    private long ReasonCount(string reason) => (SingleMarketCounter?.RejectedByReason ?? new Dictionary<string, long>()).Where(x => TradingBot.Services.PaperPhase1EligibilityLadderExporter.NormalizeReason(x.Key).Equals(reason, StringComparison.OrdinalIgnoreCase)).Sum(x => x.Value);
    public bool PaperPhase1LadderConsistent => PaperPhase1LadderPaperEligible == PaperPhase1CandidatesEligible && PaperPhase1LadderPositiveAfterSafety >= PaperPhase1LadderPaperEligible && PaperPhase1LadderValidPriced >= PaperPhase1LadderAfterSafetyComputed && PaperPhase1LadderNearBreakEven <= PaperPhase1LadderAfterSafetyComputed && (PaperPhase1LadderPaperEligible == 0 || (PaperPhase1LadderBestAfterSafetyEdge ?? decimal.MinValue) >= PaperPhase1MinEdge);

    public bool PaperPhase1ConsistencyOk => PaperPhase1ConsistencyReason == "None";
    public string PaperPhase1ConsistencyReason
    {
        get
        {
            ...
            var canaryPaper = PaperPhase1CanaryPaperOpened;
            if (PaperDiagnosticsLimitedPaperOpened != PaperClosedPositions + PaperOpenPositions) return "UnbalancedPaperOpenCloseCounts";
            if (PaperPhase1OpenAttempts != PaperPhase1OpenSucceeded + PaperPhase1OpenFailed) return "OpenAttemptAccountingMismatch";
            if (PaperPhase1PaperOpened != singlePaper + canaryPaper) return "PaperOpenedStrategySumMismatch";
            if (PaperPhase1PaperOpened != PaperDiagnosticsLimitedPaperOpened) return "PaperDiagnosticsLimitedMismatch";
            if (PaperPhase1PaperOpened > 0 && (PaperOpenPositions > 1 || PaperTotalExposure > 5m)) return "SafeLimitMismatch";
            if (otherPaper != 0) return "NonPhase1StrategyPaperOpened";
            if (!PaperPhase1CanaryConsistent) return PaperPhase1CanaryConsistencyReason;
            return "None";
        }
    }
    ...
    public string ToLogLine()
    {
        ...
        return $"[RUNTIME_HEALTH] PaperPhase1ContractFixtureCandidateInjected={PaperPhase1ContractFixtureCandidateInjected.ToString().ToLowerInvariant()} PaperPhase1ContractFixtureAffectsRuntime={PaperPhase1ContractFixtureAffectsRuntime.ToString().ToLowerInvariant()} PaperPhase1ContractFixtureIsolationOk={PaperPhase1ContractFixtureIsolationOk.ToString().ToLowerInvariant()} PaperPhase1ContractFixtureIsolationReason={PaperPhase1ContractFixtureIsolationReason} PaperPhase1ReleaseStatusEnabled={PaperPhase1ReleaseStatusEnabled.ToString().ToLowerInvariant()} PaperPhase1ReleaseStatusLogIntervalSeconds={PaperPhase1ReleaseStatusLogIntervalSeconds} PaperPhase1ReleaseStatusLogsWritten={PaperPhase1ReleaseStatusLogsWritten} PaperPhase1ReleaseStatusLogsSuppressed={PaperPhase1ReleaseStatusLogsSuppressed} PaperPhase1ReleaseStatusLastEmittedUtc={PaperPhase1ReleaseStatusLastEmittedUtc:O} PaperPhase1ReleaseStatusLastChangeReason={PaperPhase1ReleaseStatusLastChangeReason} PaperPhase1ReleaseStatusConsistent={PaperPhase1ReleaseStatusConsistent.ToString().ToLowerInvariant()} PaperPhase1ReleaseStatusExportWritten={PaperPhase1ReleaseStatusExportWritten.ToString().ToLowerInvariant()} PaperPhase1ReleaseStatusExportPath={PaperPhase1ReleaseStatusExportPath} PaperPhase1ReleaseStatusLastWriteError={PaperPhase1ReleaseStatusLastWriteError} PaperPhase1OperatorRunbookEnabled={PaperPhase1OperatorRunbookEnabled.ToString().ToLowerInvariant()} PaperPhase1OperatorRunbookIntervalSeconds={PaperPhase1OperatorRunbookIntervalSeconds} PaperPhase1OperatorRunbookLogsWritten={PaperPhase1OperatorRunbookLogsWritten} PaperPhase1OperatorRunbookLogsSuppressed={PaperPhase1OperatorRunbookLogsSuppressed} PaperPhase1OperatorRunbookLastEmittedUtc={PaperPhase1OperatorRunbookLastEmittedUtc:O} PaperPhase1OperatorRunbookLastReason={PaperPhase1OperatorRunbookLastReason} PaperPhase1OperatorRunbookConsistent={PaperPhase1OperatorRunbookConsistent.ToString().ToLowerInvariant()} PaperPhase1OperatorRunbookConsistencyReason={PaperPhase1OperatorRunbookConsistencyReason} PaperPhase1OperatorRunbookExportWritten={PaperPhase1OperatorRunbookExportWritten.ToString().ToLowerInvariant()} PaperPhase1OperatorRunbookExportPath={PaperPhase1OperatorRunbookExportPath} PaperPhase1OperatorRunbookLastWriteError={PaperPhase1OperatorRunbookLastWriteError} PaperPhase1NormalRuntimeOpened={PaperPhase1NormalRuntimeOpened} PaperPhase1NormalRuntimeClosed={PaperPhase1NormalRuntimeClosed} PaperPhase1NormalRuntimeOpenPositions={PaperPhase1NormalRuntimeOpenPositions} PaperPhase1FixtureOpened={PaperPhase1FixtureOpened} PaperPhase1FixtureClosed={PaperPhase1FixtureClosed} PaperPhase1FixtureOpenPositions={PaperPhase1FixtureOpenPositions} PaperPhase1CanaryOpened={PaperPhase1CanaryPaperOpened} PaperPhase1CanaryClosed={PaperPhase1CanaryClosed} PaperPhase1CanaryOpenPositions={PaperPhase1CanaryOpenPositions} PaperPhase1ContractFixtureEnabled={PaperPhase1ContractFixtureEnabled.ToString().ToLowerInvariant()} PaperPhase1ContractFixtureSettled={PaperPhase1ContractFixtureSettled.ToString().ToLowerInvariant()} PaperPhase1RealSettlementEnabled={PaperPhase1RealSettlementEnabled.ToString().ToLowerInvariant()} PaperPhase1RealSettlementRequested={PaperPhase1RealSettlementRequested.ToString().ToLowerInvariant()} PaperPhase1RealSettlementSucceeded={PaperPhase1RealSettlementSucceeded.ToString().ToLowerInvariant()} PaperPhase1RealSettlementFailed={PaperPhase1RealSettlementFailed.ToString().ToLowerInvariant()} PaperPhase1RealSettlementRejected={PaperPhase1RealSettlementRejected.ToString().ToLowerInvariant()} PaperPhase1RealSettlementLastPositionId={PaperPhase1RealSettlementLastPositionId} PaperPhase1RealSettlementLastReason={PaperPhase1RealSettlementLastReason} PaperPhase1RealSettlementLastRealizedPayout={PaperPhase1RealSettlementLastRealizedPayout?.ToString("0.####") ?? "N/A"} PaperPhase1RealSettlementLastRealizedPnl={PaperPhase1RealSettlementLastRealizedPnl?.ToString("0.####") ?? "N/A"} PaperPhase1RealSettlementConsistent={PaperPhase1RealSettlementConsistent.ToString().ToLowerInvariant()} PaperPhase1RealSettlementConsistencyReason={PaperPhase1RealSettlementConsistencyReason} PaperPhase1LifecycleBalanceOk={PaperPhase1LifecycleBalanceOk.ToString().ToLowerInvariant()} PaperPhase1LifecycleBalanceReason={PaperPhase1LifecycleBalanceReason} PaperPhase1ContractFixtureDryReplayOnly={PaperPhase1ContractFixtureDryReplayOnly.ToString().ToLowerInvariant()} PaperPhase1ContractFixtureAllowPaperOpen={PaperPhase1ContractFixtureAllowPaperOpen.ToString().ToLowerInvariant()} PaperPhase1ContractFixtureOpened={PaperPhase1ContractFixtureOpened.ToString().ToLowerInvariant()} PaperPhase1ContractFixtureConsistent={PaperPhase1ContractFixtureConsistent.ToString().ToLowerInvariant()} PaperPhase1ContractFixtureConsistencyReason={PaperPhase1ContractFixtureConsistencyReason} PaperPhase1RealSoakEnabled={PaperPhase1RealSoakEnabled.ToString().ToLowerInvariant()} PaperPhase1RealSoakProfileActive={PaperPhase1RealSoakProfileActive.ToString().ToLowerInvariant()} PaperPhase1RealSoakReadinessStable={PaperPhase1RealSoakReadinessStable.ToString().ToLowerInvariant()} PaperPhase1RealSoakReadinessStableMinutes={PaperPhase1RealSoakReadinessStableMinutes:0.##} PaperPhase1RealSoakOrderbookStableMinutes={PaperPhase1RealSoakOrderbookStableMinutes:0.##} PaperPhase1RealSoakMemoryStable={PaperPhase1RealSoakMemoryStable.ToString().ToLowerInvariant()} PaperPhase1RealSoakLogVolumeStable={PaperPhase1RealSoakLogVolumeStable.ToString().ToLowerInvariant()} PaperPhase1RealSoakNoCounterMismatches={PaperPhase1RealSoakNoCounterMismatches.ToString().ToLowerInvariant()} PaperPhase1RealSoakNoUnexpectedPaperOpens={PaperPhase1RealSoakNoUnexpectedPaperOpens.ToString().ToLowerInvariant()} PaperPhase1RealSoakNoSigningAttempts={PaperPhase1RealSoakNoSigningAttempts.ToString().ToLowerInvariant()} PaperPhase1RealSoakNoLiveTradingBlocks={PaperPhase1RealSoakNoLiveTradingBlocks.ToString().ToLowerInvariant()} PaperPhase1RealSoakConsistent={PaperPhase1RealSoakConsistent.ToString().ToLowerInvariant()} PaperPhase1RealSoakConsistencyReason={PaperPhase1RealSoakConsistencyReason} PaperPhase1RealAlertLevel={PaperPhase1RealAlertLevel} PaperPhase1RealAlertName={PaperPhase1RealAlertName} PaperPhase1RealAlertReason={PaperPhase1RealAlertReason} PaperPhase1RealAlertCandidateId={PaperPhase1RealAlertCandidateId} PaperPhase1RealAlertMarketId={PaperPhase1RealAlertMarketId} PaperPhase1RealAlertAfterSafetyEdge={PaperPhase1RealAlertAfterSafetyEdge?.ToString("0.####") ?? "N/A"} PaperPhase1RealAlertDistanceToMinEdge={PaperPhase1RealAlertDistanceToMinEdge?.ToString("0.####") ?? "N/A"} PaperPhase1RealAlertFirstBlockingReason={PaperPhase1RealAlertFirstBlockingReason} PaperPhase1CleanNearOpenCount={PaperPhase1CleanNearOpenCount} PaperPhase1CleanNearOpenThresholdDistance={PaperPhase1CleanNearOpenThresholdDistance:0.####} PaperPhase1CleanNearOpenBestCandidateId={PaperPhase1CleanNearOpenBestCandidateId} PaperPhase1CleanNearOpenBestAfterSafetyEdge={PaperPhase1CleanNearOpenBestAfterSafetyEdge?.ToString("0.####") ?? "N/A"} PaperPhase1CleanNearOpenBestDistanceToMinEdge={PaperPhase1CleanNearOpenBestDistanceToMinEdge?.ToString("0.####") ?? "N/A"} PaperPhase1CleanNearOpenFirstBlockingReason={PaperPhase1CleanNearOpenFirstBlockingReason} PaperPhase1CleanNearOpenAllBlockingReasons={PaperPhase1CleanNearOpenAllBlockingReasons} ProcessRunId={ProcessRunId} StartedAtUtc={StartedAtUtc:O} ScannerInstanceId={ScannerInstanceId} FormulaDiagnosticsEnabled={FormulaDiagnosticsEnabled.ToString().ToLowerInvariant()} FormulaWarningsTotal={FormulaWarningsTotal} FormulaWarningsBlocking={FormulaWarningsBlocking} FormulaWarningsNonBlocking={FormulaWarningsNonBlocking} FormulaWarningsSuppressed={FormulaWarningsSuppressed} FormulaLastWarning={FormulaLastWarning} FormulaLastWarningClassification={FormulaLastWarningClassification} FormulaDiagnosticsConsistent={FormulaDiagnosticsConsistent.ToString().ToLowerInvariant()} PaperPhase1RealWatchEnabled={PaperPhase1RealWatchEnabled.ToString().ToLowerInvariant()} PaperPhase1RealWatchProfileActive={PaperPhase1RealWatchProfileActive.ToString().ToLowerInvariant()} PaperPhase1RealWatchArmed={PaperPhase1RealWatchArmed.ToString().ToLowerInvariant()} PaperPhase1RealWatchWaitingForEdge={PaperPhase1RealWatchWaitingForEdge.ToString().ToLowerInvariant()} PaperPhase1RealWatchBestAfterSafetyEdge={PaperPhase1RealWatchBestAfterSafetyEdge?.ToString("0.####") ?? "N/A"} PaperPhase1RealWatchBestDistanceToMinEdge={PaperPhase1RealWatchBestDistanceToMinEdge?.ToString("0.####") ?? "N/A"} PaperPhase1RealWatchBestCandidateId={PaperPhase1RealWatchBestCandidateId} PaperPhase1RealWatchBestMarketId={PaperPhase1RealWatchBestMarketId} PaperPhase1RealWatchTopBlockingReason={PaperPhase1RealWatchTopBlockingReason} PaperPhase1RealWatchLastEligibleCandidateId={PaperPhase1RealWatchLastEligibleCandidateId} PaperPhase1RealWatchLastOpenAttemptUtc={PaperPhase1RealWatchLastOpenAttemptUtc:O} PaperPhase1RealWatchLastOpenResult={PaperPhase1RealWatchLastOpenResult} PaperPhase1RealWatchLastOpenBlockedReason={PaperPhase1RealWatchLastOpenBlockedReason} PaperPhase1RealWatchOpenAttempts={PaperPhase1RealWatchOpenAttempts} PaperPhase1RealWatchOpenSucceeded={PaperPhase1RealWatchOpenSucceeded} PaperPhase1RealWatchOpenFailed={PaperPhase1RealWatchOpenFailed} PaperPhase1RealWatchOpenedPositionId={PaperPhase1RealWatchOpenedPositionId} PaperPhase1RealWatchOpenPositions={PaperPhase1RealWatchOpenPositions} PaperPhase1RealWatchExposure={PaperPhase1RealWatchExposure:0.####} PaperPhase1RealWatchRealizedPnl={PaperPhase1RealWatchRealizedPnl:0.####} PaperPhase1RealWatchConsistent={PaperPhase1RealWatchConsistent.ToString().ToLowerInvariant()} PaperPhase1RealWatchConsistencyReason={PaperPhase1RealWatchConsistencyReason} PaperPhase1RealOpenPositionCount={PaperPhase1RealOpenPositionCount} PaperPhase1RealClosedPositionCount={PaperPhase1RealClosedPositionCount} PaperPhase1RealExposure={PaperPhase1RealExposure:0.####} PaperPhase1RealLocked={PaperPhase1RealLocked:0.####} PaperPhase1RealRealizedPnl={PaperPhase1RealRealizedPnl:0.####} PaperPhase1RealLastOpenedPositionId={PaperPhase1RealLastOpenedPositionId} PaperPhase1RealLastClosedPositionId={PaperPhase1RealLastClosedPositionId} PaperPhase1RealLastSettlementReason={PaperPhase1RealLastSettlementReason} PaperPhase1RealLifecycleConsistent={PaperPhase1RealLifecycleConsistent.ToString().ToLowerInvariant()} PaperPhase1RealLifecycleConsistencyReason={PaperPhase1RealLifecycleConsistencyReason} PaperPhase1PositiveReconciliationEnabled={PaperPhase1PositiveReconciliationEnabled.ToString().ToLowerInvariant()} PaperPhase1PositiveCaptureEnabled={PaperPhase1PositiveCaptureEnabled.ToString().ToLowerInvariant()} PaperPhase1PositiveCaptureThreshold={PaperPhase1PositiveCaptureThreshold:0.####} PaperPhase1PositiveCapturesTotal={PaperPhase1PositiveCapturesTotal} PaperPhase1PositiveCapturesAboveMinEdge={PaperPhase1PositiveCapturesAboveMinEdge} PaperPhase1PositiveCapturesPaperEligible={PaperPhase1PositiveCapturesPaperEligible} PaperPhase1PositiveCapturesOpened={PaperPhase1PositiveCapturesOpened} PaperPhase1PositiveBestCapturedAfterSafetyEdge={PaperPhase1PositiveBestCapturedAfterSafetyEdge?.ToString("0.####") ?? "N/A"} PaperPhase1PositiveBestCapturedCandidateId={PaperPhase1PositiveBestCapturedCandidateId} PaperPhase1PositiveBestCapturedFirstBlockingReason={PaperPhase1PositiveBestCapturedFirstBlockingReason} PaperPhase1PositiveCaptureConsistent={PaperPhase1PositiveCaptureConsistent.ToString().ToLowerInvariant()} PaperPhase1PositiveCaptureExportWritten={PaperPhase1PositiveCaptureExportWritten.ToString().ToLowerInvariant()} PaperPhase1PositiveCaptureLastWriteError={PaperPhase1PositiveCaptureLastWriteError} PaperPhase1ValidPositiveCapturesTotal={PaperPhase1ValidPositiveCapturesTotal} PaperPhase1ValidPositiveCapturesAboveMinEdge={PaperPhase1ValidPositiveCapturesAboveMinEdge} PaperPhase1ValidPositiveCapturesPaperEligible={PaperPhase1ValidPositiveCapturesPaperEligible} PaperPhase1ValidPositiveCapturesOpened={PaperPhase1ValidPositiveCapturesOpened} PaperPhase1ValidPositiveBestAfterSafetyEdge={PaperPhase1ValidPositiveBestAfterSafetyEdge?.ToString("0.####") ?? "N/A"} PaperPhase1InvalidPositiveArtifactsTotal={PaperPhase1InvalidPositiveArtifactsTotal} PaperPhase1InvalidPositiveArtifactsAboveMinEdge={PaperPhase1InvalidPositiveArtifactsAboveMinEdge} PaperPhase1InvalidPositiveArtifactsByReason={PaperPhase1InvalidPositiveArtifactsByReason} PaperPhase1InvalidPositiveArtifactBestAfterSafetyEdge={PaperPhase1InvalidPositiveArtifactBestAfterSafetyEdge?.ToString("0.####") ?? "N/A"} PaperPhase1InvalidPositiveArtifactBestCandidateId={PaperPhase1InvalidPositiveArtifactBestCandidateId} PaperPhase1InvalidPositiveArtifactBestFirstReason={PaperPhase1InvalidPositiveArtifactBestFirstReason} PaperPhase1PositiveInvalidArtifactsTotal={PaperPhase1PositiveInvalidArtifactsTotal} PaperPhase1PositiveCleanCapturesTotal={PaperPhase1PositiveCleanCapturesTotal} PaperPhase1PositiveCleanAboveMinEdge={PaperPhase1PositiveCleanAboveMinEdge} PaperPhase1PositiveCleanBestAfterSafetyEdge={PaperPhase1PositiveCleanBestAfterSafetyEdge?.ToString("0.####") ?? "N/A"} PaperPhase1PositiveCleanBestCandidateId={PaperPhase1PositiveCleanBestCandidateId} PaperPhase1PositiveCleanBestFirstBlockingReason={PaperPhase1PositiveCleanBestFirstBlockingReason} PaperPhase1PositiveRealWatchTotal={PaperPhase1PositiveRealWatchTotal} PaperPhase1PositiveRealWatchAboveMinEdge={PaperPhase1PositiveRealWatchAboveMinEdge} PaperPhase1PositiveRealWatchBestAfterSafetyEdge={PaperPhase1PositiveRealWatchBestAfterSafetyEdge?.ToString("0.####") ?? "N/A"} PaperPhase1PositiveRealWatchBestCandidateId={PaperPhase1PositiveRealWatchBestCandidateId} PaperPhase1PositiveExecutableLikeTotal={PaperPhase1PositiveExecutableLikeTotal} PaperPhase1PositiveExecutableLikeBestAfterSafetyEdge={PaperPhase1PositiveExecutableLikeBestAfterSafetyEdge?.ToString("0.####") ?? "N/A"} PaperPhase1PositivePaperEligibleTotal={PaperPhase1PositivePaperEligibleTotal} PaperPhase1PositiveOpenedTotal={PaperPhase1PositiveOpenedTotal} PaperPhase1CleanPositiveExcludedFromRealWatchTotal={PaperPhase1CleanPositiveExcludedFromRealWatchTotal} PaperPhase1CleanPositiveExcludedFromRealWatchByReason={PaperPhase1CleanPositiveExcludedFromRealWatchByReason} BestValidCleanAfterSafetyEdge={BestValidCleanAfterSafetyEdge?.ToString("0.####") ?? "N/A"} BestInvalidArtifactAfterSafetyEdge={BestInvalidArtifactAfterSafetyEdge?.ToString("0.####") ?? "N/A"} BestStrategySummaryAfterSafetyEdge={BestStrategySummaryAfterSafetyEdge?.ToString("0.####") ?? "N/A"} BestRealWatchAfterSafetyEdge={BestRealWatchAfterSafetyEdge?.ToString("0.####") ?? "N/A"} BestExecutableLikeAfterSafetyEdge={BestExecutableLikeAfterSafetyEdge?.ToString("0.####") ?? "N/A"} PaperPhase1StrategyPositiveAfterSafety={PaperPhase1StrategyPositiveAfterSafety} PaperPhase1StrategyBestAfterSafetyEdge={PaperPhase1StrategyBestAfterSafetyEdge?.ToString("0.####") ?? "N/A"} PaperPhase1StrategyBestCandidateId={PaperPhase1StrategyBestCandidateId} PaperPhase1StrategyBestCandidateReason={PaperPhase1StrategyBestCandidateReason} PaperPhase1RealWatchPositiveAfterSafety={PaperPhase1RealWatchPositiveAfterSafety} PaperPhase1PositiveCandidateMismatch={PaperPhase1PositiveCandidateMismatch.ToString().ToLowerInvariant()} PaperPhase1PositiveCandidateMismatchReason={PaperPhase1PositiveCandidateMismatchReason} PaperPhase1PositiveCandidateMismatchBlocking={PaperPhase1PositiveCandidateMismatchBlocking.ToString().ToLowerInvariant()} PaperPhase1PositiveReconciliationConsistent={PaperPhase1PositiveReconciliationConsistent.ToString().ToLowerInvariant()} PaperPhase1PositiveExcludedTotal={PaperPhase1PositiveExcludedTotal} PaperPhase1PositiveExcludedByReason={PaperPhase1PositiveExcludedByReason} PaperPhase1PositiveBestExcludedCandidateId={PaperPhase1PositiveBestExcludedCandidateId} PaperPhase1PositiveBestExcludedMarketId={PaperPhase1PositiveBestExcludedMarketId} PaperPhase1PositiveBestExcludedAfterSafetyEdge={PaperPhase1PositiveBestExcludedAfterSafetyEdge?.ToString("0.####") ?? "N/A"} PaperPhase1PositiveBestExcludedReason={PaperPhase1PositiveBestExcludedReason} PaperPhase1PositiveBestExcludedAllReasons={PaperPhase1PositiveBestExcludedAllReasons} RealPositiveAfterSafetyRaw={RealPositiveAfterSafetyRaw} RealPositiveAfterSafetyValid={RealPositiveAfterSafetyValid} RealPositiveAfterSafetyExecutableLike={RealPositiveAfterSafetyExecutableLike} RealPositiveAfterSafetyExcluded={RealPositiveAfterSafetyExcluded} RealPositiveExcludedByReason={RealPositiveExcludedByReason} RealBestPositiveAfterSafetyEdge={RealBestPositiveAfterSafetyEdge?.ToString("0.####") ?? "N/A"} RealBestPositiveCandidateId={RealBestPositiveCandidateId} RealBestPositiveFirstBlockingReason={RealBestPositiveFirstBlockingReason} RealBestPositiveAllBlockingReasons={RealBestPositiveAllBlockingReasons} DiagnosticsCounterMismatchCount={DiagnosticsCounterMismatchCount} DiagnosticsCounterMismatchBlockingCount={DiagnosticsCounterMismatchBlockingCount} DiagnosticsCounterMismatchNonBlockingCount={DiagnosticsCounterMismatchNonBlockingCount} DiagnosticsCounterMismatchLastReason={DiagnosticsCounterMismatchLastReason} DiagnosticsCounterMismatchLastBlockingReason={DiagnosticsCounterMismatchLastBlockingReason} DiagnosticsCounterMismatchLastNonBlockingReason={DiagnosticsCounterMismatchLastNonBlockingReason} DiagnosticsCounterMismatchPaperBlocking={DiagnosticsCounterMismatchPaperBlocking.ToString().ToLowerInvariant()} GlobalTradingReadiness={GlobalTradingReadiness.ToString().ToLowerInvariant()} GlobalTradingReadinessReason={GlobalTradingReadinessReason} GlobalSoakReadiness={GlobalSoakReadiness} GlobalSoakReadinessReason={GlobalSoakReadinessReason} LocalPaperPhase1Readiness={LocalPaperPhase1Readiness.ToString().ToLowerInvariant()} LocalPaperPhase1ReadinessReason={LocalPaperPhase1ReadinessReason} DiscoveryInvariantScope={DiscoveryInvariantScope} DiscoveryInvariantAppliesToGlobalReadiness={DiscoveryInvariantAppliesToGlobalReadiness.ToString().ToLowerInvariant()} DiscoveryInvariantAppliesToPaperPhase1={DiscoveryInvariantAppliesToPaperPhase1.ToString().ToLowerInvariant()} DiscoveryHealthy={DiscoveryHealthy.ToString().ToLowerInvariant()} DiscoveryReducedUniverse={DiscoveryReducedUniverse.ToString().ToLowerInvariant()} DiscoveryMode={DiscoverySelectedSource} DiscoveryScannerSafeSourceAvailable={DiscoveryScannerSafeSourceAvailable.ToString().ToLowerInvariant()} ReducedUniverseMarkets={ReducedUniverseMarkets} ReducedUniverseRawMarkets={ReducedUniverseRawMarkets} ReducedUniverseFilteredMarkets={ReducedUniverseFilteredMarkets} ReducedUniverseExcludedInvalidTokens={ReducedUniverseExcludedInvalidTokens} ReducedUniverseExcludedQuarantinedMarkets={ReducedUniverseExcludedQuarantinedMarkets} ReducedUniverseExcludedBadHistory={ReducedUniverseExcludedBadHistory} ReducedUniverseOrderbookEligibleMarkets={ReducedUniverseOrderbookEligibleMarkets} ReducedUniverseOrderbookStable={ReducedUniverseOrderbookStable.ToString().ToLowerInvariant()} ReducedUniverseScanPausedByOrderbookHealth={ReducedUniverseScanPausedByOrderbookHealth.ToString().ToLowerInvariant()} ReducedUniverseOrderbookRecoveryMode={ReducedUniverseOrderbookRecoveryMode.ToString().ToLowerInvariant()} ReducedUniverseOrderbookRecoveryCleanWindowSeconds={ReducedUniverseOrderbookRecoveryCleanWindowSeconds} ReducedUniversePostRecoveryBadRequests={ReducedUniversePostRecoveryBadRequests} MarketOrderbookQuarantineLifecycleBalanced={MarketOrderbookQuarantineLifecycleBalanced.ToString().ToLowerInvariant()} InvalidTokenQuarantineLifecycleBalanced={InvalidTokenQuarantineLifecycleBalanced.ToString().ToLowerInvariant()} OrderbookQuarantineLifecycleMismatchReason={OrderbookQuarantineLifecycleMismatchReason} InFlightBeforeBreakerCompletedAfterOpen={InFlightBeforeBreakerCompletedAfterOpen} InFlightBeforeBreakerBadRequestsAfterOpen={InFlightBeforeBreakerBadRequestsAfterOpen} TruePostBreakerNormalRequests={TruePostBreakerNormalRequests} TruePostBreakerBadRequests={TruePostBreakerBadRequests} ReducedUniverseBadHistoryActive={ReducedUniverseBadHistoryActive} ReducedUniverseBadHistoryLoaded={ReducedUniverseBadHistoryLoaded.ToString().ToLowerInvariant()} ReducedUniverseBadHistoryExpired={ReducedUniverseBadHistoryExpired} SingleMarketScanPausedByOrderbookHealth={SingleMarketScanPausedByOrderbookHealth} SingleMarketPausedCycles={SingleMarketPausedCycles} SingleMarketNormalCycles={SingleMarketNormalCycles} SingleMarketFullCyclesCompleted={SingleMarketFullCyclesCompleted} PaperPhase1Enabled={PaperPhase1Enabled.ToString().ToLowerInvariant()} PaperPhase1ProfileActive={PaperPhase1ProfileActive.ToString().ToLowerInvariant()} PaperPhase1Armed={PaperPhase1Armed.ToString().ToLowerInvariant()} PaperPhase1AllowedStrategy={PaperPhase1AllowedStrategy} PaperPhase1LiveTradingDisabled={PaperPhase1LiveTradingDisabled.ToString().ToLowerInvariant()} PaperPhase1SigningDisabled={PaperPhase1SigningDisabled.ToString().ToLowerInvariant()} PaperPhase1ReducedUniversePaperExplicitlyAllowed={PaperPhase1ReducedUniversePaperExplicitlyAllowed.ToString().ToLowerInvariant()} PaperPhase1MaxOpenPositions={PaperPhase1MaxOpenPositions} PaperPhase1MaxNotionalPerTrade={PaperPhase1MaxNotionalPerTrade:0.####} PaperPhase1MaxTotalExposure={PaperPhase1MaxTotalExposure:0.####} PaperPhase1MaxOpensPerHour={PaperPhase1MaxOpensPerHour} PaperPhase1MinEdge={PaperPhase1MinEdge:0.####} PaperPhase1Readiness={PaperPhase1Readiness.ToString().ToLowerInvariant()} PaperPhase1ReadinessReason={PaperPhase1ReadinessReason} PaperPhase1ReadinessLastEvaluatedUtc={PaperPhase1ReadinessLastEvaluatedUtc:O} PaperPhase1ReadinessEvaluationCount={PaperPhase1ReadinessEvaluationCount} PaperPhase1ReadinessUsedCurrentMemoryStable={PaperPhase1ReadinessUsedCurrentMemoryStable.ToString().ToLowerInvariant()} PaperPhase1ReadinessUsedCurrentWarmupComplete={PaperPhase1ReadinessUsedCurrentWarmupComplete.ToString().ToLowerInvariant()} PaperPhase1ReadinessUsedCurrentLogVolumeStable={PaperPhase1ReadinessUsedCurrentLogVolumeStable.ToString().ToLowerInvariant()} PaperPhase1ReadinessUsedCurrentReducedUniversePaperAllowed={PaperPhase1ReadinessUsedCurrentReducedUniversePaperAllowed.ToString().ToLowerInvariant()} PaperPhase1ReadinessStaleReasonClearedCount={PaperPhase1ReadinessStaleReasonClearedCount} PaperPhase1ReadinessCurrentBlockingReasons={PaperPhase1ReadinessCurrentBlockingReasons} PaperPhase1CandidatesSeen={PaperPhase1CandidatesSeen} PaperPhase1CandidatesRejected={PaperPhase1CandidatesRejected} PaperPhase1CandidatesEligible={PaperPhase1CandidatesEligible} PaperPhase1OpenAttempts={PaperPhase1OpenAttempts} PaperPhase1OpenSucceeded={PaperPhase1OpenSucceeded} PaperPhase1OpenFailed={PaperPhase1OpenFailed} PaperPhase1PaperOpened={PaperPhase1PaperOpened} PaperPhase1CanaryEnabled={PaperPhase1CanaryEnabled.ToString().ToLowerInvariant()} PaperPhase1CanaryProfileActive={PaperPhase1CanaryProfileActive.ToString().ToLowerInvariant()} PaperPhase1CanaryRequireProfile={PaperPhase1CanaryRequireProfile} PaperPhase1CanaryRunOnce={PaperPhase1CanaryRunOnce.ToString().ToLowerInvariant()} PaperPhase1CanaryAttempted={PaperPhase1CanaryAttempted.ToString().ToLowerInvariant()} PaperPhase1CanaryOpened={PaperPhase1CanaryOpened.ToString().ToLowerInvariant()} PaperPhase1CanarySettled={PaperPhase1CanarySettled.ToString().ToLowerInvariant()} PaperPhase1CanaryRejected={PaperPhase1CanaryRejected.ToString().ToLowerInvariant()} PaperPhase1CanaryRejectedReason={PaperPhase1CanaryRejectedReason} PaperPhase1CanaryPositionId={PaperPhase1CanaryPositionId} PaperPhase1CanaryExpectedProfit={PaperPhase1CanaryExpectedProfit:0.####} PaperPhase1CanaryRealizedPnl={PaperPhase1CanaryRealizedPnl:0.####} PaperPhase1CanaryDuplicateSuppressions={PaperPhase1CanaryDuplicateSuppressions} PaperPhase1CanaryLastDuplicateReason={PaperPhase1CanaryLastDuplicateReason} PaperPhase1CanaryOpenAttempts={PaperPhase1CanaryOpenAttempts} PaperPhase1CanaryOpenSucceeded={PaperPhase1CanaryOpenSucceeded} PaperPhase1CanaryPaperOpened={PaperPhase1CanaryPaperOpened} PaperPhase1CanaryPaperClosed={PaperPhase1CanaryPaperClosed} PaperPhase1CanaryOpenPositions={PaperPhase1CanaryOpenPositions} PaperPhase1CanaryExposure={PaperPhase1CanaryExposure:0.####} PaperPhase1CanaryLadderOpenAttempted={PaperPhase1CanaryLadderOpenAttempted} PaperPhase1CanaryLadderOpened={PaperPhase1CanaryLadderOpened} PaperPhase1CanaryLadderSettled={PaperPhase1CanaryLadderSettled} PaperPhase1CanarySyntheticOnly={PaperPhase1CanarySyntheticOnly.ToString().ToLowerInvariant()} PaperPhase1CanaryRealOrderSent={PaperPhase1CanaryRealOrderSent.ToString().ToLowerInvariant()} PaperPhase1CanarySigningAttempted={PaperPhase1CanarySigningAttempted.ToString().ToLowerInvariant()} PaperPhase1CanaryConsistent={PaperPhase1CanaryConsistent.ToString().ToLowerInvariant()} PaperPhase1CanaryConsistencyReason={PaperPhase1CanaryConsistencyReason} PaperCounterSourceOfTruth={PaperCounterSourceOfTruth} PaperCounterDuplicateSuppressions={PaperCounterDuplicateSuppressions} PaperCounterLastDuplicateExecutionId={PaperCounterLastDuplicateExecutionId} PaperCounterCountedExecutionIds={PaperCounterCountedExecutionIds} PaperCounterSyntheticCanaryCountedExecutions={PaperCounterSyntheticCanaryCountedExecutions} PaperCounterRealScannerCountedExecutions={PaperCounterRealScannerCountedExecutions} PaperCounterAuditEnabled={PaperCounterAuditEnabled.ToString().ToLowerInvariant()} PaperCounterIncrementLogsWritten={PaperCounterIncrementLogsWritten} PaperCounterDuplicateIncrementLogSuppressions={PaperCounterDuplicateIncrementLogSuppressions} PaperCounterDuplicateSuppressionLogsWritten={PaperCounterDuplicateSuppressionLogsWritten} PaperCounterLastIncrementExecutionId={PaperCounterLastIncrementExecutionId} PaperCounterLastSuppressedDuplicateExecutionId={PaperCounterLastSuppressedDuplicateExecutionId} PaperCounterAuditConsistent={PaperCounterAuditConsistent.ToString().ToLowerInvariant()} PaperPhase1LastRejectReason={PaperPhase1LastRejectReason} PaperPhase1LastEligibleCandidateId={PaperPhase1LastEligibleCandidateId} PaperPhase1LastOpenedPositionId={PaperPhase1LastOpenedPositionId} PaperPhase1LastOpenBlockedReason={PaperPhase1LastOpenBlockedReason} PaperPhase1DuplicateSuppressions={PaperPhase1DuplicateSuppressions} PaperPhase1RiskRejected={PaperPhase1RiskRejected} PaperPhase1FillRejected={PaperPhase1FillRejected} PaperPhase1DepthRejected={PaperPhase1DepthRejected} PaperPhase1EdgeRejected={PaperPhase1EdgeRejected} PaperPhase1DataQualityRejected={PaperPhase1DataQualityRejected} PaperPhase1ConsistencyOk={PaperPhase1ConsistencyOk.ToString().ToLowerInvariant()} PaperPhase1ConsistencyReason={PaperPhase1ConsistencyReason} PaperPhase1DiscoveryFallbackEnabled={PaperPhase1DiscoveryFallbackEnabled.ToString().ToLowerInvariant()} PaperPhase1DiscoveryFallbackAttempted={PaperPhase1DiscoveryFallbackAttempted.ToString().ToLowerInvariant()} PaperPhase1DiscoveryFallbackAccepted={PaperPhase1DiscoveryFallbackAccepted.ToString().ToLowerInvariant()} PaperPhase1DiscoveryFallbackRejectedReason={PaperPhase1DiscoveryFallbackRejectedReason} PaperPhase1DiscoveryFallbackSnapshotAgeHours={PaperPhase1DiscoveryFallbackSnapshotAgeHours:0.####} PaperPhase1DiscoveryFallbackSnapshotMarkets={PaperPhase1DiscoveryFallbackSnapshotMarkets} PaperPhase1DiscoveryFallbackSource={PaperPhase1DiscoveryFallbackSource} PaperPhase1UsingPersistedSnapshot={PaperPhase1UsingPersistedSnapshot.ToString().ToLowerInvariant()} PaperPhase1EligibilityLadderEnabled={PaperPhase1EligibilityLadderEnabled.ToString().ToLowerInvariant()} PaperPhase1CandidateCounterScope={PaperPhase1CandidateCounterScope} PaperPhase1LadderCounterScope={PaperPhase1LadderCounterScope} PaperPhase1LadderSeen={PaperPhase1LadderSeen} PaperPhase1LadderTokenMappingVerified={PaperPhase1LadderTokenMappingVerified} PaperPhase1LadderHasBothBooks={PaperPhase1LadderHasBothBooks} PaperPhase1LadderHasYesAsk={PaperPhase1LadderHasYesAsk} PaperPhase1LadderHasNoAsk={PaperPhase1LadderHasNoAsk} PaperPhase1LadderValidPriced={PaperPhase1LadderValidPriced} PaperPhase1LadderAfterCostComputed={PaperPhase1LadderAfterCostComputed} PaperPhase1LadderAfterSafetyComputed={PaperPhase1LadderAfterSafetyComputed} PaperPhase1LadderNearBreakEven={PaperPhase1LadderNearBreakEven} PaperPhase1LadderPositiveBeforeCost={PaperPhase1LadderPositiveBeforeCost} PaperPhase1LadderPositiveAfterCost={PaperPhase1LadderPositiveAfterCost} PaperPhase1LadderPositiveAfterSafety={PaperPhase1LadderPositiveAfterSafety} PaperPhase1LadderEdgeStable={PaperPhase1LadderEdgeStable} PaperPhase1LadderDepthSufficient={PaperPhase1LadderDepthSufficient} PaperPhase1LadderFillPassed={PaperPhase1LadderFillPassed} PaperPhase1LadderRiskPassed={PaperPhase1LadderRiskPassed} PaperPhase1LadderPaperEligible={PaperPhase1LadderPaperEligible} PaperPhase1LadderOpenAttempted={PaperPhase1LadderOpenAttempted} PaperPhase1LadderOpened={PaperPhase1LadderOpened} PaperPhase1LadderBestAfterSafetyEdge={PaperPhase1LadderBestAfterSafetyEdge?.ToString("0.####") ?? "N/A"} PaperPhase1LadderBestDistanceToMinEdge={PaperPhase1LadderBestDistanceToMinEdge?.ToString("0.####") ?? "N/A"} PaperPhase1LadderMedianDistanceToMinEdge={PaperPhase1LadderMedianDistanceToMinEdge?.ToString("0.####") ?? "N/A"} PaperPhase1LadderTopBlockingReason={PaperPhase1LadderTopBlockingReason} PaperPhase1LadderTopBlockingReasonCount={PaperPhase1LadderTopBlockingReasonCount} PaperPhase1LadderRejectedByReason={PaperPhase1LadderRejectedByReason} PaperPhase1LadderConsistent={PaperPhase1LadderConsistent.ToString().ToLowerInvariant()} PaperDiagnosticsLimitedEnabled={PaperDiagnosticsLimitedEnabled.ToString().ToLowerInvariant()} PaperDiagnosticsLimitedEligible={PaperDiagnosticsLimitedEligible.ToString().ToLowerInvariant()} PaperDiagnosticsLimitedBlockedReason={PaperDiagnosticsLimitedBlockedReason} PaperDiagnosticsLimitedAllowedStrategy={PaperDiagnosticsLimitedAllowedStrategy} PaperDiagnosticsLimitedMaxOpenPositions={PaperDiagnosticsLimitedMaxOpenPositions} PaperDiagnosticsLimitedMaxPaperNotionalPerTrade={PaperDiagnosticsLimitedMaxPaperNotionalPerTrade:0.####} PaperDiagnosticsLimitedMaxPaperTotalExposure={PaperDiagnosticsLimitedMaxPaperTotalExposure:0.####} PaperDiagnosticsLimitedOpensLastHour={PaperDiagnosticsLimitedOpensLastHour} PaperDiagnosticsLimitedGateLastRejectReason={PaperDiagnosticsLimitedGateLastRejectReason} PaperDiagnosticsLimitedPaperOpened={PaperDiagnosticsLimitedPaperOpened} OrderbookStableNow={OrderbookStableNow.ToString().ToLowerInvariant()} OrderbookStableWindowMinutes={OrderbookStableWindowMinutes} BatchBookBadRequestsDeltaWindow={BatchBookBadRequestsDeltaWindow} BatchBookInvalidTokensDeltaWindow={BatchBookInvalidTokensDeltaWindow} PostBreakerBadRequestsDeltaWindow={PostBreakerBadRequestsDeltaWindow} ReducedUniverseOrderbookStableNow={ReducedUniverseOrderbookStableNow.ToString().ToLowerInvariant()} ReducedUniverseMaxMarkets={ReducedUniverseMaxMarkets} ReducedUniverseSource={ReducedUniverseSource} PaperExecutionGloballyBlockedByDiscovery={PaperExecutionGloballyBlockedByDiscovery.ToString().ToLowerInvariant()} PaperBlockedByDiscoveryMode={PaperBlockedByDiscoveryMode} StrategyExecutionGloballyBlocked={StrategyExecutionGloballyBlocked.ToString().ToLowerInvariant()} DiagnosticsUniverse={DiagnosticsUniverse} AllowReducedUniverseDiagnosticsOnly={AllowReducedUniverseDiagnosticsOnly.ToString().ToLowerInvariant()} ReducedUniverseRequireExplicitFlag={ReducedUniverseRequireExplicitFlag.ToString().ToLowerInvariant()} ReducedUniverseExplicitFlagSatisfied={ReducedUniverseExplicitFlagSatisfied.ToString().ToLowerInvariant()} ReducedUniverseActivationBlockedReason={ReducedUniverseActivationBlockedReason} TradingReadiness={TradingReadiness.ToString().ToLowerInvariant()} SignalRPayloadTrimmedTotal={SignalRPayloadTrimmedTotal} SignalRPayloadTrimmedLogged={SignalRPayloadTrimmedLogged} SignalRPayloadTrimmedSuppressed={SignalRPayloadTrimmedSuppressed} SignalRPayloadTrimmedLastEvent={SignalRPayloadTrimmedLastEvent} SignalRPayloadTrimmedLastItemsBefore={SignalRPayloadTrimmedLastItemsBefore} SignalRPayloadTrimmedLastItemsAfter={SignalRPayloadTrimmedLastItemsAfter} SignalRPayloadTrimmedSummaryIntervalSeconds={SignalRPayloadTrimmedSummaryIntervalSeconds} SignalRLogNoiseControlEnabled={SignalRLogNoiseControlEnabled.ToString().ToLowerInvariant()} SignalRLogNoiseControlConsistent={SignalRLogNoiseControlConsistent.ToString().ToLowerInvariant()} SoakReadiness={SoakReadiness} SoakReadinessReason={SoakReadinessReason} LongRunStable={LongRunStable.ToString().ToLowerInvariant()} DiscoveryStable={DiscoveryStable.ToString().ToLowerInvariant()} PaperPretradeRejects={PaperPretradeRejects} PaperExecutions={PaperExecutionsCount} LiveTradingBlocked={LiveTradingBlockedCount} SigningAttempts={SigningAttempts} BatchBookRequests={BatchBookRequests} BatchBookBadRequests={BatchBookBadRequests} BatchBookInvalidTokens={BatchBookInvalidTokens} SingleMarketRawCandidates={SingleMarketRawCandidates} SingleMarketDataQualityRejected={SingleMarketDataQualityRejected} SingleMarketDataQualityRejectedRawPositive={SingleMarketDataQualityRejectedRawPositive} SingleMarketBelowMinEdge={SingleMarketBelowMinEdge} SingleMarketPositiveBeforeCost={SingleMarketPositiveBeforeCost} SingleMarketPositiveAfterCost={SingleMarketPositiveAfterCost} SingleMarketPositiveAfterSafety={SingleMarketPositiveAfterSafety} SingleMarketValidRawPositive={SingleMarketValidRawPositive} SingleMarketValidAfterCostPositive={SingleMarketValidAfterCostPositive} SingleMarketValidAfterSafetyPositive={SingleMarketValidAfterSafetyPositive} SingleMarketEdgeStable={SingleMarketEdgeStable} SingleMarketExecutionReady={SingleMarketExecutionReady} SingleMarketRejectedByFill={SingleMarketRejectedByFill} SingleMarketRejectedByDepth={SingleMarketRejectedByDepth} SingleMarketRejectedByRisk={SingleMarketRejectedByRisk} SingleMarketRejectedByPaperDiagnosticsLimitedGate={SingleMarketRejectedByPaperDiagnosticsLimitedGate} SingleMarketBestRawEdge={SingleMarketBestRawEdge?.ToString("0.####") ?? "N/A"} SingleMarketBestAfterCostEdge={SingleMarketBestAfterCostEdge?.ToString("0.####") ?? "N/A"} SingleMarketBestAfterSafetyEdge={SingleMarketBestAfterSafetyEdge?.ToString("0.####") ?? "N/A"} SingleMarketBestExecutableEdge={SingleMarketBestExecutableEdge?.ToString("0.####") ?? "N/A"} SingleMarketBestRejectedReason={SingleMarketBestRejectedReason} SingleMarketValidEdgeSamples={SingleMarketValidEdgeSamples} SingleMarketEdgeDistributionSampleMode={SingleMarketEdgeDistributionSampleMode} SingleMarketEdgeDistributionCapacity={SingleMarketEdgeDistributionCapacity} SingleMarketEdgeDistributionDroppedSamples={SingleMarketEdgeDistributionDroppedSamples} SingleMarketRawEdgeMin={Fmt(SingleMarketRawEdgeMin)} SingleMarketRawEdgeP01={Fmt(SingleMarketRawEdgeP01)} SingleMarketRawEdgeP05={Fmt(SingleMarketRawEdgeP05)} SingleMarketRawEdgeP10={Fmt(SingleMarketRawEdgeP10)} SingleMarketRawEdgeP25={Fmt(SingleMarketRawEdgeP25)} SingleMarketRawEdgeP50={Fmt(SingleMarketRawEdgeP50)} SingleMarketRawEdgeP75={Fmt(SingleMarketRawEdgeP75)} SingleMarketRawEdgeP90={Fmt(SingleMarketRawEdgeP90)} SingleMarketRawEdgeP95={Fmt(SingleMarketRawEdgeP95)} SingleMarketRawEdgeP99={Fmt(SingleMarketRawEdgeP99)} SingleMarketRawEdgeMax={Fmt(SingleMarketRawEdgeMax)} SingleMarketAfterCostEdgeMin={Fmt(SingleMarketAfterCostEdgeMin)} SingleMarketAfterCostEdgeP01={Fmt(SingleMarketAfterCostEdgeP01)} SingleMarketAfterCostEdgeP05={Fmt(SingleMarketAfterCostEdgeP05)} SingleMarketAfterCostEdgeP10={Fmt(SingleMarketAfterCostEdgeP10)} SingleMarketAfterCostEdgeP25={Fmt(SingleMarketAfterCostEdgeP25)} SingleMarketAfterCostEdgeP50={Fmt(SingleMarketAfterCostEdgeP50)} SingleMarketAfterCostEdgeP75={Fmt(SingleMarketAfterCostEdgeP75)} SingleMarketAfterCostEdgeP90={Fmt(SingleMarketAfterCostEdgeP90)} SingleMarketAfterCostEdgeP95={Fmt(SingleMarketAfterCostEdgeP95)} SingleMarketAfterCostEdgeP99={Fmt(SingleMarketAfterCostEdgeP99)} SingleMarketAfterCostEdgeMax={Fmt(SingleMarketAfterCostEdgeMax)} SingleMarketAfterSafetyEdgeMin={Fmt(SingleMarketAfterSafetyEdgeMin)} SingleMarketAfterSafetyEdgeP01={Fmt(SingleMarketAfterSafetyEdgeP01)} SingleMarketAfterSafetyEdgeP05={Fmt(SingleMarketAfterSafetyEdgeP05)} SingleMarketAfterSafetyEdgeP10={Fmt(SingleMarketAfterSafetyEdgeP10)} SingleMarketAfterSafetyEdgeP25={Fmt(SingleMarketAfterSafetyEdgeP25)} SingleMarketAfterSafetyEdgeP50={Fmt(SingleMarketAfterSafetyEdgeP50)} SingleMarketAfterSafetyEdgeP75={Fmt(SingleMarketAfterSafetyEdgeP75)} SingleMarketAfterSafetyEdgeP90={Fmt(SingleMarketAfterSafetyEdgeP90)} SingleMarketAfterSafetyEdgeP95={Fmt(SingleMarketAfterSafetyEdgeP95)} SingleMarketAfterSafetyEdgeP99={Fmt(SingleMarketAfterSafetyEdgeP99)} SingleMarketAfterSafetyEdgeMax={Fmt(SingleMarketAfterSafetyEdgeMax)} SingleMarketAfterSafetyEdgeBelowMinus5bp={SingleMarketAfterSafetyEdgeBelowMinus5bp} SingleMarketAfterSafetyEdgeMinus5bpToMinus2bp={SingleMarketAfterSafetyEdgeMinus5bpToMinus2bp} SingleMarketAfterSafetyEdgeMinus2bpToMinus1bp={SingleMarketAfterSafetyEdgeMinus2bpToMinus1bp} SingleMarketAfterSafetyEdgeMinus1bpTo0={SingleMarketAfterSafetyEdgeMinus1bpTo0} SingleMarketAfterSafetyEdge0To1bp={SingleMarketAfterSafetyEdge0To1bp} SingleMarketAfterSafetyEdge1bpTo5bp={SingleMarketAfterSafetyEdge1bpTo5bp} SingleMarketAfterSafetyEdgeAbove5bp={SingleMarketAfterSafetyEdgeAbove5bp} OrderbookCircuitBreakerActive={OrderbookCircuitBreakerActive.ToString().ToLowerInvariant()} Strategies={{{strategies}}} StrategyPaperOpened={{{strategyPaperOpened}}} OpportunityFamilyRankingEnabled={OpportunityFamilyRankingEnabled.ToString().ToLowerInvariant()} OpportunityFamilyBuckets={OpportunityFamilyBuckets} OpportunityFamilyBestPricedFamily={OpportunityFamilyBestPricedFamily} OpportunityFamilyBestPricedAfterSafetyEdge={OpportunityFamilyBestPricedAfterSafetyEdge?.ToString("0.####") ?? "N/A"} OpportunityFamilyBestUnpricedFamily={OpportunityFamilyBestUnpricedFamily} OpportunityFamilyBestUnpricedVerificationScore={OpportunityFamilyBestUnpricedVerificationScore} OpportunityFamilyClosestToBreakEvenCount={OpportunityFamilyClosestToBreakEvenCount} OpportunityFamilyPositiveFamilies={OpportunityFamilyPositiveFamilies} OpportunityFamilyExecutableFamilies={OpportunityFamilyExecutableFamilies} OpportunityFamilyInvalidRawSpikeFamilies={OpportunityFamilyInvalidRawSpikeFamilies} OpportunityFamilyInvalidRawSpikeBestEdge={OpportunityFamilyInvalidRawSpikeBestEdge?.ToString("0.####") ?? "N/A"} OpportunityFamilyInvalidRawSpikeTopReason={OpportunityFamilyInvalidRawSpikeTopReason} OpportunityFamilyRankingConsistent={OpportunityFamilyRankingConsistent.ToString().ToLowerInvariant()} OpportunityFamilyRankingConsistencyReason={OpportunityFamilyRankingConsistencyReason} FocusUniverseEnabled={FocusUniverseEnabled.ToString().ToLowerInvariant()} FocusUniverseWatchlistSize={FocusUniverseWatchlistSize} FocusUniverseAdmitted={FocusUniverseAdmitted} FocusUniverseEvicted={FocusUniverseEvicted} FocusUniverseRefreshed={FocusUniverseRefreshed} FocusUniverseSkippedByOrderbookHealth={FocusUniverseSkippedByOrderbookHealth} FocusUniverseImproving={FocusUniverseImproving} FocusUniverseWorsening={FocusUniverseWorsening} FocusUniverseStable={FocusUniverseStable} FocusUniverseBestStrategy={FocusUniverseBestStrategy} FocusUniverseBestAfterSafetyEdge={FocusUniverseBestAfterSafetyEdge?.ToString("0.####") ?? "N/A"} FocusUniverseBestEdgeDelta={FocusUniverseBestEdgeDelta?.ToString("0.####") ?? "N/A"} FocusUniverseClosestToBreakEvenCount={FocusUniverseClosestToBreakEvenCount} FocusUniverseExecutionReady={FocusUniverseExecutionReady} FocusUniversePaperOpened={FocusUniversePaperOpened} FocusUniverseConsistent={FocusUniverseConsistent.ToString().ToLowerInvariant()} EdgeTransitionEnabled={EdgeTransitionEnabled.ToString().ToLowerInvariant()} EdgeTransitionTracked={EdgeTransitionTracked} EdgeTransitionNew={EdgeTransitionNew} EdgeTransitionImproving={EdgeTransitionImproving} EdgeTransitionWorsening={EdgeTransitionWorsening} EdgeTransitionStableNearBreakEven={EdgeTransitionStableNearBreakEven} EdgeTransitionAlertCandidates={EdgeTransitionAlertCandidates} EdgeTransitionPositiveCandidates={EdgeTransitionPositiveCandidates} EdgeTransitionInvalidated={EdgeTransitionInvalidated} EdgeTransitionStale={EdgeTransitionStale} EdgeTransitionBestStrategy={EdgeTransitionBestStrategy} EdgeTransitionBestCurrentEdge={EdgeTransitionBestCurrentEdge?.ToString("0.####") ?? "N/A"} EdgeTransitionBestDeltaFromFirst={EdgeTransitionBestDeltaFromFirst?.ToString("0.####") ?? "N/A"} EdgeTransitionBestDeltaFromPrevious={EdgeTransitionBestDeltaFromPrevious?.ToString("0.####") ?? "N/A"} EdgeTransitionLastAlertReason={EdgeTransitionLastAlertReason} EdgeTransitionConsistent={EdgeTransitionConsistent.ToString().ToLowerInvariant()} EdgeCompressionEnabled={EdgeCompressionEnabled.ToString().ToLowerInvariant()} EdgeCompressionItems={EdgeCompressionItems} EdgeCompressionNearBreakEven={EdgeCompressionNearBreakEven} EdgeCompressionCompressing={EdgeCompressionCompressing} EdgeCompressionExpanding={EdgeCompressionExpanding} EdgeCompressionFlat={EdgeCompressionFlat} EdgeCompressionRawPositive={EdgeCompressionRawPositive} EdgeCompressionAfterCostPositive={EdgeCompressionAfterCostPositive} EdgeCompressionAfterSafetyPositive={EdgeCompressionAfterSafetyPositive} EdgeCompressionBlockedByMarketSpread={EdgeCompressionBlockedByMarketSpread} EdgeCompressionBlockedByCost={EdgeCompressionBlockedByCost} EdgeCompressionBlockedBySafety={EdgeCompressionBlockedBySafety} EdgeCompressionBestStrategy={EdgeCompressionBestStrategy} EdgeCompressionBestRawEdge={EdgeCompressionBestRawEdge?.ToString("0.####") ?? "N/A"} EdgeCompressionBestAfterCostEdge={EdgeCompressionBestAfterCostEdge?.ToString("0.####") ?? "N/A"} EdgeCompressionBestAfterSafetyEdge={EdgeCompressionBestAfterSafetyEdge?.ToString("0.####") ?? "N/A"} EdgeCompressionBestDistanceToBreakEven={EdgeCompressionBestDistanceToBreakEven?.ToString("0.####") ?? "N/A"} EdgeCompressionMedianDistanceToBreakEven={EdgeCompressionMedianDistanceToBreakEven?.ToString("0.####") ?? "N/A"} EdgeCompressionP95AfterSafetyEdge={EdgeCompressionP95AfterSafetyEdge?.ToString("0.####") ?? "N/A"} EdgeCompressionDominantDragComponent={EdgeCompressionDominantDragComponent} EdgeCompressionConsistent={EdgeCompressionConsistent.ToString().ToLowerInvariant()} SpreadMicrostructureEnabled={SpreadMicrostructureEnabled.ToString().ToLowerInvariant()} SpreadMicrostructureItems={SpreadMicrostructureItems} SpreadMicrostructureSkippedByOrderbookHealth={SpreadMicrostructureSkippedByOrderbookHealth} SpreadMicrostructureWideAskSpread={SpreadMicrostructureWideAskSpread} SpreadMicrostructureThinTopBook={SpreadMicrostructureThinTopBook} SpreadMicrostructureBothWideAndThin={SpreadMicrostructureBothWideAndThin} SpreadMicrostructureAlreadyNearExecutable={SpreadMicrostructureAlreadyNearExecutable} SpreadMicrostructureDepthSufficient={SpreadMicrostructureDepthSufficient} SpreadMicrostructureBestStrategy={SpreadMicrostructureBestStrategy} SpreadMicrostructureBestAfterSafetyEdge={SpreadMicrostructureBestAfterSafetyEdge?.ToString("0.####") ?? "N/A"} SpreadMicrostructureBestMoveNeededToBreakEven={SpreadMicrostructureBestMoveNeededToBreakEven?.ToString("0.####") ?? "N/A"} SpreadMicrostructureMedianMoveNeededToBreakEven={SpreadMicrostructureMedianMoveNeededToBreakEven?.ToString("0.####") ?? "N/A"} SpreadMicrostructureP95MoveNeededToBreakEven={SpreadMicrostructureP95MoveNeededToBreakEven?.ToString("0.####") ?? "N/A"} SpreadMicrostructureMinTicksToBreakEven={SpreadMicrostructureMinTicksToBreakEven?.ToString() ?? "N/A"} SpreadMicrostructureMedianTicksToBreakEven={SpreadMicrostructureMedianTicksToBreakEven?.ToString("0.####") ?? "N/A"} SpreadMicrostructureDominantCause={SpreadMicrostructureDominantCause} DiagnosticsDashboardHistoryEnabled={TradingBot.Services.DiagnosticsDashboardHistoryService.CurrentTrend.Enabled.ToString().ToLowerInvariant()} DiagnosticsDashboardHistorySamples={TradingBot.Services.DiagnosticsDashboardHistoryService.CurrentTrend.Samples} DiagnosticsDashboardHistoryOldestSampleUtc={(TradingBot.Services.DiagnosticsDashboardHistoryService.CurrentTrend.OldestSampleUtc.HasValue ? TradingBot.Services.DiagnosticsDashboardHistoryService.CurrentTrend.OldestSampleUtc.Value.ToString("O") : "")} DiagnosticsDashboardHistoryNewestSampleUtc={(TradingBot.Services.DiagnosticsDashboardHistoryService.CurrentTrend.NewestSampleUtc.HasValue ? TradingBot.Services.DiagnosticsDashboardHistoryService.CurrentTrend.NewestSampleUtc.Value.ToString("O") : "")} DiagnosticsDashboardHistoryEdgeBestDeltaFromOldest={TradingBot.Services.DiagnosticsDashboardHistoryService.CurrentTrend.EdgeBestDeltaFromOldest?.ToString("0.####") ?? "N/A"} DiagnosticsDashboardHistoryEdgeBestDeltaFromPrevious={TradingBot.Services.DiagnosticsDashboardHistoryService.CurrentTrend.EdgeBestDeltaFromPrevious?.ToString("0.####") ?? "N/A"} DiagnosticsDashboardHistoryMoveNeededDeltaFromOldest={TradingBot.Services.DiagnosticsDashboardHistoryService.CurrentTrend.MoveNeededDeltaFromOldest?.ToString("0.####") ?? "N/A"} DiagnosticsDashboardHistoryMoveNeededDeltaFromPrevious={TradingBot.Services.DiagnosticsDashboardHistoryService.CurrentTrend.MoveNeededDeltaFromPrevious?.ToString("0.####") ?? "N/A"} DiagnosticsDashboardHistorySignalRTrimSuppressedDelta={TradingBot.Services.DiagnosticsDashboardHistoryService.CurrentTrend.SignalRTrimSuppressedDelta ?? 0} DiagnosticsDashboardHistoryFocusWatchlistDelta={TradingBot.Services.DiagnosticsDashboardHistoryService.CurrentTrend.FocusWatchlistDelta ?? 0} DiagnosticsDashboardHistoryOrderbookBadRequestsDelta={TradingBot.Services.DiagnosticsDashboardHistoryService.CurrentTrend.OrderbookBadRequestsDelta ?? 0} DiagnosticsDashboardHistoryConsistent={TradingBot.Services.DiagnosticsDashboardHistoryService.CurrentTrend.Consistent.ToString().ToLowerInvariant()} SpreadMicrostructureConsistent={SpreadMicrostructureConsistent.ToString().ToLowerInvariant()} RuntimeProfile={RuntimeProfile} ActiveStrategies={ActiveStrategies} PaperEligibleStrategies={PaperEligibleStrategies} ShadowStrategies={ShadowStrategies} Uptime={Uptime}";
    }
    ...
    {
        state.ApplyReadinessInvariantCorrections(options?.MarketDiscovery.SourceAuditOnly ?? false, options);
        ...
        return new RuntimeHealthSnapshot(
            ...
            PaperCounterSourceOfTruth: state.PaperCounterSourceOfTruth,
            PaperCounterDuplicateSuppressions: state.PaperCounterDuplicateSuppressions,
            PaperCounterLastDuplicateExecutionId: state.PaperCounterLastDuplicateExecutionId,
            PaperCounterCountedExecutionIds: state.PaperCounterCountedExecutionIds,
            PaperCounterSyntheticCanaryCountedExecutions: state.PaperCounterSyntheticCanaryCountedExecutions,
            PaperCounterRealScannerCountedExecutions: state.PaperCounterRealScannerCountedExecutions,
            PaperCounterAuditEnabled: state.PaperCounterAuditEnabled,
            PaperCounterIncrementLogsWritten: state.PaperCounterIncrementLogsWritten,
            PaperCounterDuplicateIncrementLogSuppressions: state.PaperCounterDuplicateIncrementLogSuppressions,
            PaperCounterDuplicateSuppressionLogsWritten: state.PaperCounterDuplicateSuppressionLogsWritten,
            PaperCounterLastIncrementExecutionId: state.PaperCounterLastIncrementExecutionId,
            PaperCounterLastSuppressedDuplicateExecutionId: state.PaperCounterLastSuppressedDuplicateExecutionId,
            PaperCounterAuditConsistent: state.PaperCounterAuditConsistent,
    ...
    private static string PaperDiagnosticsLimitedReason(BotRuntimeState state, TradingBot.Options.TradingBotOptions? options)
    {
        ...
        if (DiagnosticsCounterMismatchPaperBlockingForState(state, options)) reasons.Add("DiagnosticsCounterMismatch");
    ...
    private static bool IsPaperDiagnosticsLimitedEligible(BotRuntimeState state, TradingBot.Options.TradingBotOptions? options)
    {
        ...
        return state.AllowReducedUniverseDiagnosticsOnly
            ...
            && stats.TruePostBreakerBadRequests == 0
            && !DiagnosticsCounterMismatchPaperBlockingForState(state, options)
    ...
    private static bool DiagnosticsCounterMismatchPaperBlockingForState(BotRuntimeState state, TradingBot.Options.TradingBotOptions? options)
    {
        if (state.DiagnosticsCounterMismatchCount <= 0) return false;
        var expectedReducedUniverseGlobalBlock = options is not null
            ...
            && state.AllowReducedUniverseDiagnosticsOnly
            && state.ReducedUniverseExplicitFlagSatisfied
            && !state.TradingReadiness
            && (string.Equals(state.SoakReadiness, "Blocked", StringComparison.OrdinalIgnoreCase) || string.Equals(state.SoakReadiness, "BlockedReducedUniverseDiagnosticsOnly", StringComparison.OrdinalIgnoreCase))
            && state.DiagnosticsCounterMismatchLastReason.Contains("UnhealthyDiscoveryRequiresUnstableRuntime", StringComparison.OrdinalIgnoreCase);
        return !expectedReducedUniverseGlobalBlock;
    }
...
public static class RuntimeHealthTrendTracker
{
    ...
    public static string ToSoakStatusLogLine(RuntimeHealthSnapshot health, RuntimeHealthTrend trend, TradingBot.Options.TradingBotOptions? options = null, BotRuntimeState? state = null)
    {
        ...
        if (!paperDiagnosticsLimitedEligibleNow && string.IsNullOrWhiteSpace(paperDiagnosticsLimitedBlockedReasonNow))
        {
            paperDiagnosticsLimitedBlockedReasonNow = "UnknownEligibilityBlock";
            Console.WriteLine($"[PAPER_DIAGNOSTICS_LIMITED_EMPTY_BLOCK_REASON_WARNING] ProcessRunId={health.ProcessRunId}");
        }
