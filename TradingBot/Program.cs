        ...
        try
        {
            ...
            if (options.MultiOutcomeArbitrage.Enabled)
            {
                ...
                if (options.MultiOutcomeArbitrage.EvaluateVerifiedGroupsAgainstFullPool)
                {
                    ...
                    var allowlistHealth = resolved.Select(g => new { groupKey = g.GroupKey, enabled = true, configuredMarketCount = g.MarketIds.Count, resolvedMarketCount = g.ResolvedMarkets.Count, missingMarketCount = g.MissingMarketIds.Count + g.MissingConditionIds.Count, missingMarketIds = g.MissingMarketIds, foundMarketIds = g.ResolvedMarkets.Select(x => x.id).ToArray(), status = g.ValidationStatus == "VerifiedGroupResolved" ? "Healthy" : "Broken", reason = g.RejectionReason ?? (g.ValidationStatus == "VerifiedGroupResolved" ? "Healthy" : "Unresolved"), recommendedAction = g.ValidationStatus == "VerifiedGroupResolved" ? "PruneIfSafe" : (g.ResolvedMarkets.Count < 2 ? "DisableMissingMarkets" : "RefreshFromCandidateExport") }).ToArray();
