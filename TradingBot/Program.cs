...
static async Task RunScannerAsync(BotRuntimeState state, IBotUiLogger uiLogger, IHubContext<BotHub> hub, VerifiedBasketExecutionCoordinator verifiedExecution, VerifiedBasketDryRunOrderBuilder dryRunBuilder, DryRunFillSimulator fillSimulator, AllowlistRepairService allowlistRepairService, AllowlistRepairLockProvider lockProvider, MemoryGuard memoryGuard, DiagnosticsDashboardService diagnosticsDashboard, DiagnosticsDashboardHistoryService diagnosticsDashboardHistory, PaperPhase1RealWatchService paperPhase1RealWatch, QuietLogGate quietLogGate, ExecutionOptions executionOptions, TradingBotOptions options, OpportunityFilteringOptions filtering, string contentRootPath, CancellationToken stoppingToken)
{
    ...
    string ReducedUniverseSnapshotPath()
    {
        var configured = options.PaperPhase1DiscoveryFallback.SnapshotPath;
        if (string.IsNullOrWhiteSpace(configured)) configured = Path.Combine("exports", "reduced-universe-last-healthy-snapshot.json");
        return Path.IsPathRooted(configured) ? configured : Path.Combine(contentRootPath, configured);
    }
    ...
    void PersistHealthyReducedUniverseSnapshot(IReadOnlyList<Market> markets, ReducedUniverseOrderbookFilterResult filter)
    {
        ...
        var stable = filter.Markets.Count > 0
            ...
            && stats.BatchInvalidTokens == 0
            && runtimeHealth.OrderbookStableNow
            && runtimeHealth.ReducedUniverseOrderbookStableNow
            && stats.InvalidTokenQuarantineActive == 0
            && stats.MarketOrderbookQuarantineActive == 0
            && stats.OrderbookCircuitBreakerState.Equals("Closed", StringComparison.OrdinalIgnoreCase);
        if (!profileOk || !stable) return;
        var unique = markets.Select(m => m.id).Where(x => !string.IsNullOrWhiteSpace(x)).Distinct(StringComparer.OrdinalIgnoreCase).Count() == markets.Count;
        var validTokens = markets.All(m => m.clobTokenIds.Count >= 2 && m.clobTokenIds.All(t => !string.IsNullOrWhiteSpace(t)));
        var path = ReducedUniverseSnapshotPath();
        var payload = new ReducedUniverseHealthySnapshot(
            DateTime.UtcNow,
            ProcessRunContext.ProcessRunId,
            "GammaOffset",
            markets.Count,
            filter.EligibleMarkets,
            "ReducedUniverseDiagnosticsOnly",
            markets.Select(m => new ReducedUniverseHealthyMarket(m.id, m.conditionId ?? string.Empty, m.question, string.Empty, string.Empty, string.Empty, m.active ?? true, m.closed ?? false, m.archived ?? false, m.clobTokenIds.ToArray(), m.outcomes.ToArray(), DateTime.UtcNow)).ToArray(),
            new ReducedUniverseHealthyIntegrity(unique, validTokens, markets.Count == filter.Markets.Count));
    ...
    bool TryActivatePaperPhase1ReducedUniverseFallback(MarketDiscoverySummary attemptSummary, int activeDiscoveryMarkets, out string rejectReason)
    {
        rejectReason = "None";
        ...
        discoveryPersistedSnapshotActiveMarkets = discoveredMarkets.Count;
        discoveryMode = "PersistedReducedUniverseSnapshot";
        lastDiscoverySummary = attemptSummary with { DiscoveryHealthy = false, DiscoveryMode = "PersistedReducedUniverseSnapshot", StoppedReason = "PaperPhase1PersistedReducedUniverseSnapshot" };
        scannerPausedByDiscoveryGuard = false;
        Console.WriteLine($"[PAPER_PHASE1_DISCOVERY_FALLBACK_ACCEPTED] SnapshotAgeHours={ageHours:0.##} Markets={discoveredMarkets.Count} Source=PersistedReducedUniverseSnapshot Reason=LiveDiscoveryTimeout ProcessRunId={ProcessRunContext.ProcessRunId}");
        return true;
    }
    ...
    void UpdateDiscoveryGuardRuntimeState()
    {
        ...
        var paperPhase1SnapshotFallbackActive = string.Equals(discoveryMode, "PersistedReducedUniverseSnapshot", StringComparison.OrdinalIgnoreCase);
        var reducedUniverseActive = options.MarketDiscovery.AllowReducedUniverseDiagnosticsOnly && (string.Equals(discoveryMode, "ReducedUniverseDiagnosticsOnly", StringComparison.OrdinalIgnoreCase) || paperPhase1SnapshotFallbackActive);
        ...
        discoveryScannerSafeSourceAvailable = paperPhase1SnapshotFallbackActive || (!reducedUniverseActive && (discoveryUsingLastHealthySnapshot || (lastDiscoverySummary.DiscoveryHealthy && !string.Equals(discoveryMode, "Blocked", StringComparison.OrdinalIgnoreCase))));
        var discoveryBlocked = !paperPhase1SnapshotFallbackActive && (reducedUniverseActive || options.MarketDiscovery.SourceAuditOnly || !lastDiscoverySummary.DiscoveryHealthy || string.Equals(discoveryMode, "Blocked", StringComparison.OrdinalIgnoreCase) || !discoveryBootstrapHealthy || !discoveryScannerSafeSourceAvailable);
        var discoveryBlockedReason = options.MarketDiscovery.SourceAuditOnly
            ? "SourceAuditOnly"
            : reducedUniverseActive
                ? (paperPhase1SnapshotFallbackActive ? "SnapshotFallbackMode" : reducedUniverseBuilt ? "BlockedReducedUniverseDiagnosticsOnly" : "ReducedUniverseSourceUnavailable")
        ...
        if (paperPhase1SnapshotFallbackActive) longRunReasons.Add("SnapshotFallbackMode");
        else if (reducedUniverseActive) longRunReasons.Add("BlockedReducedUniverseDiagnosticsOnly");
        ...
        state.SetDiscoveryGuardState(
            ...
            soakReadiness: paperPhase1SnapshotFallbackActive ? "SnapshotFallbackMode" : discoveryBlocked || !reducedUniverseOrderbookStable ? "Blocked" : "Ready",
            soakReadinessReason: paperPhase1SnapshotFallbackActive ? "PersistedReducedUniverseSnapshot" : !reducedUniverseOrderbookStable ? "ReducedUniverseOrderbookUnstable" : discoveryBlocked ? discoveryBlockedReason : "None",
            discoveryBlockedReason: discoveryBlockedReason,
            discoverySelectedSource: paperPhase1SnapshotFallbackActive ? "PersistedReducedUniverseSnapshot" : reducedUniverseActive ? "ReducedUniverseDiagnosticsOnly" : discoveryScannerSafeSourceAvailable ? (discoveryUsingLastHealthySnapshot ? "PersistedHealthySnapshot" : discoveryMode) : "Blocked",
            ...
            reducedUniverseMaxMarkets: options.MarketDiscovery.ReducedUniverseMaxMarkets,
            reducedUniverseSource: paperPhase1SnapshotFallbackActive ? "PersistedReducedUniverseSnapshot" : options.MarketDiscovery.ReducedUniverseSource,
            paperExecutionGloballyBlockedByDiscovery: !paperPhase1SnapshotFallbackActive && (reducedUniverseActive || !lastDiscoverySummary.DiscoveryHealthy || !discoveryScannerSafeSourceAvailable || string.Equals(discoveryMode, "Blocked", StringComparison.OrdinalIgnoreCase)),
            strategyExecutionGloballyBlocked: !paperPhase1SnapshotFallbackActive && (reducedUniverseActive || discoveryBlocked),
            diagnosticsUniverse: reducedUniverseActive ? "Reduced" : "Full",
            tradingReadiness: !paperPhase1SnapshotFallbackActive && !discoveryBlocked && reducedUniverseOrderbookStable,
            allowReducedUniverseDiagnosticsOnly: options.MarketDiscovery.AllowReducedUniverseDiagnosticsOnly,
            reducedUniverseRequireExplicitFlag: options.MarketDiscovery.ReducedUniverseRequireExplicitFlag,
    ...
    if (options.MarketDiscovery.SourceAuditOnly)
    {
        ...
        PaperPhase1ReleaseStatusService.Update(phase1Health, contentRootPath, options);
    ...
    while (!stoppingToken.IsCancellationRequested)
    {
        ...
        try
        {
            ...
            if (lastDiscoveryAt == default || DateTime.UtcNow - lastDiscoveryAt >= TimeSpan.FromMinutes(options.FullDiscoveryIntervalMinutes) || discoveredMarkets.Count == 0)
            {
                ...
                var reducedUniverseDiscoveryRequested = !options.MarketDiscovery.SourceAuditOnly && options.MarketDiscovery.AllowReducedUniverseDiagnosticsOnly;
                var reducedUniverseAvailable = reducedUniverseDiscoveryRequested && discoveredCandidateMarkets.Count > 0;
                if (reducedUniverseAvailable)
                {
                    discoveryPartialAttemptCount++;
                ...
                else
                {
                    ...
                    {
                        if (reducedUniverseDiscoveryRequested)
                        {
                            if (!TryActivatePaperPhase1ReducedUniverseFallback(attemptSummary, discoveredCandidateMarkets.Count, out var phase1FallbackRejectReason))
                            {
                                discoveredMarkets = new List<Market>();
                                lastDiscoverySummary = attemptSummary with { DiscoveryHealthy = false, DiscoveryMode = "ReducedUniverseDiagnosticsOnly", StoppedReason = "ReducedUniverseSourceUnavailable" };
                                discoveryMode = "ReducedUniverseDiagnosticsOnly";
                                scannerPausedByDiscoveryGuard = true;
                                discoveryGuardSkippedCycles++;
                                discoveryLastFailureReason = $"{discoveryLastFailureReason};PaperPhase1DiscoveryFallbackAttempted=true;PaperPhase1DiscoveryFallbackAccepted=false;PaperPhase1DiscoveryFallbackRejectedReason={phase1FallbackRejectReason}";
                            ...
                            {
                                discoveryLastFailureReason = $"{discoveryLastFailureReason};PaperPhase1DiscoveryFallbackAttempted=true;PaperPhase1DiscoveryFallbackAccepted=true;PaperPhase1DiscoveryFallbackRejectedReason=None";
                            }
                        }
                        else
                        {
            ...
            if (options.MarketDiscovery.AllowReducedUniverseDiagnosticsOnly && (string.Equals(discoveryMode, "ReducedUniverseDiagnosticsOnly", StringComparison.OrdinalIgnoreCase) || string.Equals(discoveryMode, "PersistedReducedUniverseSnapshot", StringComparison.OrdinalIgnoreCase)))
            {
            ...
            var pauseReducedOrderbookScan = options.MarketDiscovery.AllowReducedUniverseDiagnosticsOnly
                && (string.Equals(discoveryMode, "ReducedUniverseDiagnosticsOnly", StringComparison.OrdinalIgnoreCase) || string.Equals(discoveryMode, "PersistedReducedUniverseSnapshot", StringComparison.OrdinalIgnoreCase))
            ...
            if (!pauseReducedOrderbookScan
                ...
                && filtered.Count > 0)
                PersistHealthyReducedUniverseSnapshot(discoveredMarkets, reducedUniverseFilter);
            ...
            if (options.MultiOutcomeArbitrage.Enabled)
            {
                ...
                if (options.MultiOutcomeArbitrage.EvaluateVerifiedGroupsAgainstFullPool)
                {
                    ...
                    var autoPricingHealth = RuntimeHealthSnapshot.From(state, options);
                    var autoOrderbookHealthClean = (!options.AutoCandidatePricing.RequireOrderbookStableNow || autoPricingHealth.OrderbookStableNow) && (!options.AutoCandidatePricing.RequireReducedUniverseOrderbookStableNow || autoPricingHealth.ReducedUniverseOrderbookStableNow) && autoPricingHealth.OrderbookCircuitBreakerState == "Closed";
                    var autoVerification = await AutoCandidateVerificationService.VerifyAsync(multiOutcomeReport.CandidateGroupsForReview, multiOutcomeValidator.GetAllowlistedGroups(), discoveredMarkets, orderbookService, orderbookSemaphore, options.MultiOutcomeArbitrage, options.AutoCandidatePricing, autoOrderbookHealthClean, autoPricingHealth.PaperDiagnosticsLimitedEligible, autoConfig.Mode, state.ProcessRunId, stoppingToken);
                    ...
                    var autoBestPriced = autoVerification.PricingSucceeded > 0;
                    var autoShadowWouldOpen = autoConfig.Mode == StrategyMode.ShadowPaperEligible ? autoVerification.ShadowWouldOpen : 0;
                    var autoBestAfterSafetyEdge = autoVerification.BestAfterSafetyEdge;
                    if (autoConfig.Mode == StrategyMode.ShadowPaperEligible && autoShadowWouldOpen > 0)
                        Console.WriteLine($"[SHADOW_PAPER_CANDIDATE] Strategy=AutoCandidateMultiOutcome GroupKey={autoVerification.Best?.GroupKey ?? "N/A"} VerificationScore={autoVerification.Best?.VerificationScore ?? 0} AfterSafetyEdge={(autoBestAfterSafetyEdge.HasValue ? autoBestAfterSafetyEdge.Value.ToString("0.####") : "N/A")} WouldOpen=true BlockedReason=ShadowMode");
                    ...
                    if (ShouldQuietLog("auto-candidate-pricing", "AUTO_CANDIDATE_PRICING_SUMMARY", $"{autoVerification.PricingAttempted}|{autoVerification.PricingSucceeded}|{autoVerification.PricingSkippedIncomplete}|{autoVerification.PricingSkippedByHealth}|{autoVerification.ShadowWouldOpen}", LogImportance.Normal, everyNCycles: options.Diagnostics.OperationalQuietMode ? 10 : 1))
                        Console.WriteLine($"[AUTO_CANDIDATE_PRICING_SUMMARY] Candidates={autoVerification.Candidates.Count} VerifiedLike={autoVerification.Candidates.Count(x => x.VerificationConfidence is "High" or "Medium" && x.VerificationCategory != "AutoCandidateDifferentEvent" && x.VerificationCategory != "AutoCandidateUnverified")} PricingAttempted={autoVerification.PricingAttempted} PricingSucceeded={autoVerification.PricingSucceeded} PricingFailed={autoVerification.PricingFailed} PricingSkippedIncomplete={autoVerification.PricingSkippedIncomplete} PricingSkippedByHealth={autoVerification.PricingSkippedByHealth} CompletedFromVerifiedAllowlist={autoVerification.CompletedFromVerifiedAllowlist} CompletedFromCandidatePool={autoVerification.CompletedFromCandidatePool} CompletedFromDiscoveryPool={autoVerification.CompletedFromDiscoveryPool} BestRawEdge={(autoVerification.BestRawEdge.HasValue ? autoVerification.BestRawEdge.Value.ToString("0.####") : "N/A")} BestAfterSafetyEdge={(autoVerification.BestAfterSafetyEdge.HasValue ? autoVerification.BestAfterSafetyEdge.Value.ToString("0.####") : "N/A")} ShadowWouldOpen={autoVerification.ShadowWouldOpen}");
                    strategyOrchestrator.RecordExternalResult(new OpportunityStrategyScanResult(
                        ...
                        BlockedByOrderbookHealth: autoVerification.PricingSkippedByHealth,
                        BlockedByPaperDiagnosticsLimitedGate: autoVerification.Candidates.Count(x => x.BlockedReason == "PaperDiagnosticsLimitedGate"),
                        ...
                        BestCandidateExecutableLike: autoVerification.Candidates.Any(x => x.ExecutableLike),
                        ...
                        AutoCandidatePricingAttempted: autoVerification.PricingAttempted,
                        AutoCandidatePricingSucceeded: autoVerification.PricingSucceeded,
                        AutoCandidatePricingFailed: autoVerification.PricingFailed,
                        AutoCandidatePricingSkippedByHealth: autoVerification.PricingSkippedByHealth,
                        AutoCandidatePricingSkippedIncomplete: autoVerification.PricingSkippedIncomplete,
                        AutoCandidatePricingMissingNoAsk: autoVerification.PricingMissingNoAsk,
                        AutoCandidatePricingMissingYesAsk: autoVerification.PricingMissingYesAsk,
                        AutoCandidatePricingEmptyBook: autoVerification.PricingEmptyBook,
                        AutoCandidateBestRawEdge: autoVerification.BestRawEdge,
                        AutoCandidateBestAfterCostEdge: autoVerification.BestAfterCostEdge,
                        AutoCandidateBestAfterSafetyEdge: autoVerification.BestAfterSafetyEdge,
                        AutoCandidateBestPricingReason: autoVerification.BestPricingReason,
                        AutoCandidateCompletedGroups: autoVerification.CompletedGroups,
                        AutoCandidateIncompleteGroups: autoVerification.IncompleteGroups),
                    ...
                    strategyOrchestrator.RecordExternalResult(new OpportunityStrategyScanResult(
                        ...
                        PositiveEdges: verifiedActivePositive,
                        ...
                        ValidPriced: verifiedBestStrategyEdge.HasValue && (activeExecutable > 0 || verifiedWouldOpenIfPaperEligible > 0) ? Math.Max(1, activeExecutable + verifiedWouldOpenIfPaperEligible) : 0,
                        InvalidOrUnpriced: Math.Max(0, verifiedEvaluated - (verifiedBestStrategyEdge.HasValue ? 1 : 0)),
                        DataQualityRejected: verifiedRejectedByReason.Where(x => x.Key.Contains("DataQuality", StringComparison.OrdinalIgnoreCase) || x.Key.Contains("Suspicious", StringComparison.OrdinalIgnoreCase)).Sum(x => x.Value),
                        Unverified: verifiedRejectedByReason.Where(x => x.Key.Contains("Unverified", StringComparison.OrdinalIgnoreCase)).Sum(x => x.Value),
                        ReviewOnly: finalReviewOnlyCount,
                        MissingPricing: verifiedRejectedByReason.Where(x => IsMissingPricingReason(x.Key)).Sum(x => x.Value),
                        BestCandidateValid: verifiedBestStrategyEdge.HasValue && (activeExecutable > 0 || verifiedWouldOpenIfPaperEligible > 0),
                        BestCandidatePriced: verifiedBestStrategyEdge.HasValue,
                        BestCandidateExecutableLike: activeExecutable > 0 || verifiedWouldOpenIfPaperEligible > 0,
                        BestCandidateReason: (activeExecutable > 0 || verifiedWouldOpenIfPaperEligible > 0) ? "Priced" : (verifiedTopSkip.Key ?? (verifiedBestStrategyEdge.HasValue ? "Priced" : "N/A"))),
                    ...
                    var shadowHealth = RuntimeHealthSnapshot.From(state, options);
                    ...
                    if (autoConfig.Mode == StrategyMode.ShadowPaperEligible)
                    {
                        var autoShadowRowsAdded = false;
                        foreach (var row in autoVerification.Candidates.Where(x => x.WouldShadowOpen || (x.VerificationConfidence is "High" or "Medium" && x.MissingLegCount > 0)).Take(25))
                        {
                            autoShadowRowsAdded = true;
                            shadowRows.Add(new { timestampUtc = DateTime.UtcNow, processRunId = state.ProcessRunId, strategy = "AutoCandidateMultiOutcome", mode = autoConfig.Mode.ToString(), marketOrGroupKey = row.GroupKey, legs = row.PresentLegCount, expectedLegs = row.ExpectedLegCount, missingLegs = row.MissingLegCount, verificationScore = row.VerificationScore, verificationConfidence = row.VerificationConfidence, rawEdge = row.RawEdge, afterCostEdge = row.AfterCostEdge, afterSafetyEdge = row.AfterSafetyEdge, executableQty = 0, notionalAtCap = 0, wouldOpen = row.WouldShadowOpen, blockedReason = row.WouldShadowOpen ? "ShadowMode" : row.BlockedReason, paperDiagnosticsLimitedEligible = shadowHealth.PaperDiagnosticsLimitedEligible, orderbookStableNow = shadowHealth.OrderbookStableNow, reducedUniverseOrderbookStableNow = shadowHealth.ReducedUniverseOrderbookStableNow });
                        }
                    ...
                    if (!options.Diagnostics.OperationalQuietMode || ShouldQuietLog("opportunity-family-ranking", "OPPORTUNITY_FAMILY_RANKING", $"{familyRanking.Buckets}|{familyRanking.BestPricedFamily}|{familyRanking.BestPricedAfterSafetyEdge}|{familyRanking.BestUnpricedFamily}|{familyRanking.TopRecommendedAction}|{familyRanking.InvalidRawSpikeFamiliesCount}|{familyRanking.RankingConsistent}", LogImportance.Normal, familyLog, everyNCycles: 1, maxPerHour: 12))
                        Console.WriteLine(familyLog);
...
static void SyncRuntimeState(BotRuntimeState state, OpportunityMonitor monitor, PaperPositionBook pb, string executionJournalPath, ExecutionPolicy p, OrderBookService obs, PaperTradingEngine paper, int marketsScanned, DateTime scanStart, string? lastError, SingleMarketScanStats scanStats, OpportunityFilteringOptions filtering, MarketDiscoverySummary discovery, int rollingOffset, int batchSize, int totalDiscovered, DateTime discoveryStartedAt, DateTime discoveryCompletedAt, int emptyCycles, int configuredMarketScanLimit, int effectiveMarketLimit, int configuredMaxMarketsToDiscover, TradingBotOptions options, string poolLimitReason, MultiOutcomeGroupArbEngine.MultiOutcomeScanReport multiOutcomeReport, string contentRootPath, PaperPhase1SyntheticCanaryService paperPhase1Canary)
{
    ...
    PaperPhase1ReleaseStatusService.Update(RuntimeHealthSnapshot.From(state, options), contentRootPath, options);
...
static void EmitMultiStrategySummary(BotRuntimeState state, TradingBotOptions options)
{
    ...
    var bestValidPriced = configured
        .Where(x => x.Counter!.BestCandidateValid && x.Counter!.BestCandidatePriced && x.Counter!.BestEdge.HasValue)
        .OrderByDescending(x => x.Counter!.BestEdge!.Value)
        .FirstOrDefault();
    var bestExecutable = configured
        .Where(x => x.Counter!.BestCandidateValid && x.Counter!.BestCandidatePriced && x.Counter!.BestEdge.HasValue && (x.Counter!.ExecutionReady > 0 || x.Counter!.ShadowWouldOpen > 0))
        .OrderByDescending(x => x.Counter!.BestEdge!.Value)
        .FirstOrDefault();
...
static string InvalidCategory(string? reason)
    => string.IsNullOrWhiteSpace(reason) ? "N/A"
        : IsMissingPricingReason(reason) ? "MissingPricing"
        : reason.Contains("Unverified", StringComparison.OrdinalIgnoreCase) ? "Unverified"
        : reason.Contains("ReviewOnly", StringComparison.OrdinalIgnoreCase) ? "ReviewOnly"
...
public sealed record ReducedUniverseHealthySnapshot(DateTime GeneratedAtUtc, string ProcessRunId, string Source, int MarketCount, int OrderbookEligibleMarkets, string DiscoveryMode, IReadOnlyList<ReducedUniverseHealthyMarket> Markets, ReducedUniverseHealthyIntegrity Integrity);
public sealed record ReducedUniverseHealthyMarket(string MarketId, string ConditionId, string Question, string Slug, string EventSlug, string Category, bool Active, bool Closed, bool Archived, IReadOnlyList<string> TokenIds, IReadOnlyList<string> Outcomes, DateTime LastSeenUtc);
public sealed record ReducedUniverseHealthyIntegrity(bool UniqueMarketIds, bool ValidTokenIds, bool MarketCountMatches);
