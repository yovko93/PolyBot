...
static async Task RunScannerAsync(BotRuntimeState state, IBotUiLogger uiLogger, IHubContext<BotHub> hub, VerifiedBasketExecutionCoordinator verifiedExecution, VerifiedBasketDryRunOrderBuilder dryRunBuilder, ExecutionOptions executionOptions, TradingBotOptions options, OpportunityFilteringOptions filtering, string contentRootPath, CancellationToken stoppingToken)
{
    ...
    http.Timeout = TimeSpan.FromMilliseconds(Math.Max(options.ExternalApiTimeoutSeconds * 1000, options.MarketDiscovery.RequestTimeoutMs + 5000));
    ...
    while (!stoppingToken.IsCancellationRequested)
    {
        ...
        try
        {
            ...
            if (lastDiscoveryAt == default || DateTime.UtcNow - lastDiscoveryAt >= TimeSpan.FromMinutes(options.FullDiscoveryIntervalMinutes) || discoveredMarkets.Count == 0)
            {
                ...
                var discoveryHealth = DiscoveryHealthFactory.FromSummary(lastDiscoverySummary, executionOptions.RequireHealthyDiscoveryForPaperOpen);
                state.SetDiscoveryHealth(discoveryHealth);
                ...
                File.WriteAllText(discoveryHealthPath, System.Text.Json.JsonSerializer.Serialize(discoveryHealth, new System.Text.Json.JsonSerializerOptions { WriteIndented = true }));
            ...
            if (options.MultiOutcomeArbitrage.Enabled)
            {
                ...
                if (options.MultiOutcomeArbitrage.EvaluateVerifiedGroupsAgainstFullPool)
                {
                    ...
                    foreach (var g in resolved)
                    {
                        ...
                        if (isExec)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    verifiedExecution.Audit(new ExecutionAuditEvent(DateTime.UtcNow, opp.Id, opp.GroupKey, opp.Strategy, "DryRunOrderPlanRejected", "Rejected", "DiscoveryUnhealthy", pre.NetEdge, pre.ExpectedProfit, pre.EstimatedCost, pre.Quantity, $"Pages={lastDiscoverySummary.PagesFetched};Active={lastDiscoverySummary.ActiveMarketsAvailable}"));
                                    Console.WriteLine($"[DRY_RUN_ORDER_PLAN_REJECTED] Group={opp.GroupKey} Reason=DiscoveryUnhealthy");
                                    continue;
                                }
                                ...
                                verifiedExecution.Audit(new ExecutionAuditEvent(DateTime.UtcNow, opp.Id, opp.GroupKey, opp.Strategy, "DryRunOrderPlanCreated", "Ok", lastDiscoverySummary.DiscoveryHealthy ? "DryRunOnly" : "DryRunOnly;DiscoveryDegraded", pre.NetEdge, pre.ExpectedProfit, pre.EstimatedCost, pre.Quantity, $"Orders={plan.Orders.Count};ScanConfidence={lastDiscoverySummary.ScanConfidence}"));
                                Console.WriteLine($"[DRY_RUN_ORDER_PLAN_CREATED] Group={opp.GroupKey} Orders={plan.Orders.Count} Qty={pre.Quantity:0.####} TotalCost={plan.TotalEstimatedCost:0.####} ExpectedProfit={plan.ExpectedProfit:0.####} DryRunOnly=true ScanConfidence={lastDiscoverySummary.ScanConfidence}");
                                if (executionOptions.RequireHealthyDiscoveryForPaperOpen && !lastDiscoverySummary.DiscoveryHealthy && !executionOptions.AllowPaperExecutionOnPartialDiscovery)
                                {
                                    verifiedExecution.Audit(new ExecutionAuditEvent(DateTime.UtcNow, opp.Id, opp.GroupKey, opp.Strategy, "PreTradeBlocked", "Blocked", "DiscoveryUnhealthy", pre.NetEdge, pre.ExpectedProfit, pre.EstimatedCost, pre.Quantity, $"Pages={lastDiscoverySummary.PagesFetched};Active={lastDiscoverySummary.ActiveMarketsAvailable}"));
                                    Console.WriteLine($"[VERIFIED_PRETRADE_BLOCKED] Group={opp.GroupKey} Reason=DiscoveryUnhealthy Pages={lastDiscoverySummary.PagesFetched} Active={lastDiscoverySummary.ActiveMarketsAvailable}");
                                    Console.WriteLine($"[PAPER_OPEN_BLOCKED] Group={opp.GroupKey} Reason=DiscoveryUnhealthy");
                                    continue;
                                }
                    ...
                    var allowlistHealth = allowlistedGroups.Select(g =>
                    {
                        ...
                        {
                            return new { groupKey = g.GroupKey, enabled = g.Enabled, configuredMarketCount = g.MarketIds.Count, resolvedMarketCount = 0, missingMarketCount = g.MarketIds.Count, missingMarketIds = g.MarketIds, foundMarketIds = Array.Empty<string>(), status = lastDiscoverySummary.DiscoveryHealthy ? "BrokenAllowlist" : "MissingBecauseDiscoveryIncomplete", reason = lastDiscoverySummary.DiscoveryHealthy ? "ResolverMissingConfiguredGroup" : "PartialDiscovery", recommendedAction = lastDiscoverySummary.DiscoveryHealthy ? "RefreshFromCandidateExport" : "RetryDiscovery/WaitForHealthyDiscovery" };
                        }
                        return new { groupKey = rg.GroupKey, enabled = g.Enabled, configuredMarketCount = rg.MarketIds.Count, resolvedMarketCount = rg.ResolvedMarkets.Count, missingMarketCount = rg.MissingMarketIds.Count + rg.MissingConditionIds.Count, missingMarketIds = rg.MissingMarketIds, foundMarketIds = rg.ResolvedMarkets.Select(x => x.id).ToArray(), status = rg.ValidationStatus == "VerifiedGroupResolved" ? "Healthy" : (lastDiscoverySummary.DiscoveryHealthy ? "BrokenAllowlist" : "MissingBecauseDiscoveryIncomplete"), reason = rg.RejectionReason ?? (rg.ValidationStatus == "VerifiedGroupResolved" ? "Healthy" : (lastDiscoverySummary.DiscoveryHealthy ? "Unresolved" : "PartialDiscovery")), recommendedAction = rg.ValidationStatus == "VerifiedGroupResolved" ? "PruneIfSafe" : (lastDiscoverySummary.DiscoveryHealthy ? (rg.ResolvedMarkets.Count < 2 ? "DisableMissingMarkets" : "RefreshFromCandidateExport") : "RetryDiscovery/WaitForHealthyDiscovery") };
                    }).ToArray();
                    var healthyCount = allowlistHealth.Count(x=>x.status=="Healthy");
                    var brokenCount = allowlistHealth.Count(x=>x.status=="Broken" || x.status=="BrokenAllowlist");
                    ...
                    var scanMetrics = VerifiedScanMetrics.Summarize(snapshot.VerifiedBaskets, "PolymarketApprox", options.MultiOutcomeArbitrage.MinMultiOutcomeEdge);
                    var bestExperimentalNet = scanMetrics.BestExperimentalNet;
                    var bestAlternateProfileNet = scanMetrics.BestAlternateProfileNet ?? decimal.MinValue;
                    ...
                    var scanConfidence = lastDiscoverySummary.ScanConfidence;
                    verifiedScanCycle++;
                    var verifiedScanFingerprint = $"{multiOutcomeValidator.LoadedAllowlistCount}|{verifiedResolved}|{unresolved}|{verifiedEvaluated}|{verifiedExecutable}|{verifiedMismatch}|{bestConservativeNet}|{bestExperimentalNet}|{bestAlternateProfileNet}|{bestRaw}|{scanConfidence}|{lastDiscoverySummary.DiscoveryHealthy}";
                    ...
                    var bestExperimentalText = bestExperimentalNet.HasValue ? bestExperimentalNet.Value.ToString() : "N/A";
                    if (shouldLogVerifiedScan) Console.WriteLine($"[MULTI_VERIFIED_SCAN] Configured={multiOutcomeValidator.LoadedAllowlistCount} Resolved={verifiedResolved} Unresolved={unresolved} Evaluated={verifiedEvaluated} ActiveExecutable={verifiedExecutable} ExperimentalCandidates={scanMetrics.ExperimentalCandidateCount} DiagnosticsOnlyPositive={snapshot.DiagnosticsOnlyPositive.Count} Mismatch={verifiedMismatch} BestActiveNet={(bestConservativeNet.HasValue ? bestConservativeNet.Value : 0m)} BestExperimentalNet={bestExperimentalText} BestAlternateProfileNet={bestAlternateProfileNet} BestRaw={bestRaw} ScanConfidence={scanConfidence} DiscoveryHealthy={lastDiscoverySummary.DiscoveryHealthy.ToString().ToLowerInvariant()}");
